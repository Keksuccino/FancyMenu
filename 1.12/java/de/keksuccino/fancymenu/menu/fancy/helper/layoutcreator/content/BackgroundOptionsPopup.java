	...
	public void render(int mouseX, int mouseY, GuiScreen renderIn) {
		...
		if (this.isDisplayed()) {
			...
			if (s.equals(Locals.localize("helper.creator.backgroundoptions.backgroundimage"))) {
				...
				this.panoramaButton.drawButton(Minecraft.getMinecraft(), mouseX, mouseY, Minecraft.getMinecraft().getRenderPartialTicks());

				this.noPanoramaButton.x = (renderIn.width / 2) + 5;
				this.noPanoramaButton.y = (renderIn.height / 2) + 30;
				this.noPanoramaButton.drawButton(Minecraft.getMinecraft(), mouseX, mouseY, Minecraft.getMinecraft().getRenderPartialTicks());
			}
