...
public class ButtonActionPopup extends Popup {
	...
	public ButtonActionPopup(Consumer<String> contentCallback, Consumer<Integer> typeCallback, int type) {
		...
		this.messageButton = new AdvancedButton(0, 0, 100, 20, m, true, (press) -> {
			this.type = 0;
			press.displayString = "§aSend Message";
			this.linkButton.displayString = "Open Weblink";
		});
		this.addButton(this.messageButton);
		
		this.linkButton = new AdvancedButton(0, 0, 100, 20, l, true, (press) -> {
			this.type = 1;
			press.displayString = "§aOpen Weblink";
			this.messageButton.displayString = "Send Message";
		});
	...
	public void render(int mouseX, int mouseY, GuiScreen renderIn) {
		...
		if (this.isDisplayed()) {
			...
			renderIn.drawCenteredString(Minecraft.getMinecraft().fontRenderer, "§lButton Configuration", renderIn.width / 2, (renderIn.height / 2) - (height / 2) - 40, Color.WHITE.getRGB());
			
			renderIn.drawCenteredString(Minecraft.getMinecraft().fontRenderer, "Action Value [Link or Message/Command]:", renderIn.width / 2, (renderIn.height / 2) - 45, Color.WHITE.getRGB());
			
			this.textField.x = (renderIn.width / 2) - (this.textField.width / 2);
			this.textField.y = (renderIn.height / 2) - 25;
			this.textField.drawTextBox();
			
			renderIn.drawCenteredString(Minecraft.getMinecraft().fontRenderer, "Action Type:", renderIn.width / 2, ((renderIn.height / 2) + (height / 2)) - 45, Color.WHITE.getRGB());
			
			this.messageButton.x = (renderIn.width / 2) - (this.messageButton.width + 5);
			this.messageButton.y = ((renderIn.height / 2) + (height / 2)) - this.messageButton.height - 5;
			
			this.linkButton.x = (renderIn.width / 2) + 5;
			this.linkButton.y = ((renderIn.height / 2) + (height / 2)) - this.linkButton.height - 5;
