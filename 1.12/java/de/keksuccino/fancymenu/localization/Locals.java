...
public class Locals {
	...
	private static void copyLocalsFile(String language) {
		...
		try {
			try {
				br = new BufferedReader(new InputStreamReader(Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("keksuccino", "fmlocals/" + language + ".local")).getInputStream(), StandardCharsets.UTF_8));
	...
	public static String localize(String key, String... dynamicValues) {
		String playerLang = Minecraft.getMinecraft().gameSettings.language;
