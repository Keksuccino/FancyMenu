...
public class MixinFilePackResources {
    ...
    /** @reason Minecraft creates a double-slash prefix when the generic pack API enumerates an archive namespace root, which otherwise hides every root and nested resource in that namespace. */
    ...
    private boolean wrap_listResourcesPrefix_FancyMenu(String resourcePath, String prefix, Operation<Boolean> original, PackType type, String namespace, String directory, PackResources.ResourceOutput output) {
        // Minecraft appends a slash to an already slash-terminated namespace root when the generic API is asked to list from "". Without this correction archive packs match "assets/<namespace>//" and report nothing.
