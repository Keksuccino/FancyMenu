...
final class ClientResourceIndexBuilder {
    ...
    private static void applyFilter(@NotNull PackResources pack, @NotNull Set<ResourceLocation> locations) {
        ...
        try {
            filter = pack.getMetadataSection(ResourceFilterSection.SERIALIZER);
        } catch (Exception ex) {
    ...
    private static void collectPackLocations(@NotNull PackResources pack, @NotNull Set<ResourceLocation> locations) {
        ...
        for (String namespace : namespaces) {
            if (namespace == null || ResourceLocation.tryBuild(namespace, "valid") == null) {
                LOGGER.warn("[FANCYMENU] Ignoring invalid client resource namespace '{}' reported by pack '{}'.", namespace, getPackName(pack));
                continue;
            }
            try {
                Collection<ResourceLocation> packLocations = pack.getResources(PackType.CLIENT_RESOURCES, namespace, "", location -> true);
                if (packLocations != null) packLocations.forEach(location -> collectLocation(pack, namespace, location, locations));
            } catch (RuntimeException ex) {
