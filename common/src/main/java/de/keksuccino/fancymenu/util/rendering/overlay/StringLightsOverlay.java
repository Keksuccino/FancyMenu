...
public class StringLightsOverlay extends AbstractWidget implements NavigatableWidget {
    ...
    private void renderStrings(GuiGraphicsExtractor graphics, int overlayX, int overlayY, int overlayWidth, int overlayHeight) {
    ...
    private void renderStringLine(GuiGraphicsExtractor graphics, int overlayX, int overlayY, int overlayWidth, int overlayHeight, StringLight light, float controlX, float controlY, float thickness, int color) {
    ...
    private void renderBulbs(GuiGraphicsExtractor graphics, int overlayX, int overlayY, int overlayWidth, int overlayHeight, StringLight light, float controlX, float controlY, boolean christmasMode, int baseRgb, float baseAlphaScale) {
        for (LightBulb bulb : light.bulbs) {
    ...
    private int applyBrightness(int rgb, float brightness) {
        int red = Mth.clamp(Mth.floor(ARGB.red(rgb) * brightness), 0, 255);
        int green = Mth.clamp(Mth.floor(ARGB.green(rgb) * brightness), 0, 255);
        int blue = Mth.clamp(Mth.floor(ARGB.blue(rgb) * brightness), 0, 255);
        return (red << 16) | (green << 8) | blue;
    }

    private void fillClamped(GuiGraphicsExtractor graphics, float minX, float minY, float maxX, float maxY, int overlayX, int overlayY, int overlayWidth, int overlayHeight, int color) {
