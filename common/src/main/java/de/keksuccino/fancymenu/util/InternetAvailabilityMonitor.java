    ...
    void init() {
        ...
        synchronized (this.lifecycleLock) {
            if (initializationFailure == null && createdTask != null && this.lifecycle == Lifecycle.RUNNING && this.generation == scheduledGeneration) {
                ...
                // Treat a scheduler creation or registration failure as terminal so a later init cannot race teardown
                // or reuse the closed probe after partially publishing lifecycle ownership.
