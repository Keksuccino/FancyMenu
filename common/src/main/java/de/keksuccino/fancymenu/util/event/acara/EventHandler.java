...
public class EventHandler {
	...
	public void postEvent(EventBase event) {
		...
		synchronized (this.events) {
			...
			listeners = new ArrayList<>(registeredListeners);
		}
	...
	/**
	 * This will register all public static event listener methods of the given class annotated with {@link de.keksuccino.fancymenu.util.event.acara.EventListener}.<br>
	...
	/**
	 * This will register all public non-static event listener methods of the given object annotated with {@link de.keksuccino.fancymenu.util.event.acara.EventListener}.<br>
	 * Event listener methods need to have only ONE parameter and this parameter has to be the event type (subclass of {@link EventBase}).<br><br>
	 *
	 * <b>Example of a valid event listener method:</b>
	 *
	...
	protected boolean hasEventListenerAnnotation(@NotNull EventMethod m) {
		for (Annotation a : m.annotations) {
			if (a instanceof de.keksuccino.fancymenu.util.event.acara.EventListener) return true;
		}
		return false;
	}
		...
		protected int tryGetPriority() {
			try {
				for (Annotation a : this.annotations) {
					if (a instanceof de.keksuccino.fancymenu.util.event.acara.EventListener) {
