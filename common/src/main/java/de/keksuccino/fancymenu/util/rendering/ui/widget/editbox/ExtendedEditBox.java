...
public class ExtendedEditBox extends EditBox implements UniqueWidget, NavigatableWidget, FancyMenuWidget {
        ...
        if (this.isVisible()) {
            ...
            if (!renderSmallCursor && access.getSuggestionFancyMenu() != null) {
                if (renderWithUiBase) {
                    ...
                    graphics.text(this.font, access.getSuggestionFancyMenu(), (int) (finalTextXAfterCursor - 1), (int) textY, this.suggestionTextColor.getColorInt(), this.textShadow);
                }
            }
    ...
    private void renderFancyHighlight(@NotNull GuiGraphicsExtractor graphics, int minX, int minY, int maxX, int maxY, int textColor) {
        if (minX < maxX) {
            int i = minX;
            minX = maxX;
            maxX = i;
