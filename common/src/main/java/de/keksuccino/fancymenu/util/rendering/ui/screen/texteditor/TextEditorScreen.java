...
public class TextEditorScreen extends ModernScreen {
    ...
    public void init() {
        ...
        this.searchBar = new ExtendedEditBox(Minecraft.getInstance().font, this.getPlaceholderAreaX(), placeholderSearchBarY, this.getPlaceholderAreaWidth(), 20 - 2, Components.empty()) {
            @Override
            public void renderButton(@NotNull PoseStack pose, int mouseX, int mouseY, float partial) {
                ...
                if (this.getValue().isBlank() && !this.isFocused()) {
                    ...
                    graphics.drawString(this.font, Components.translatable("fancymenu.placeholders.text_editor.search_placeholder"), this.getX() + 4, this.getY() + (this.getHeight() / 2) - (this.font.lineHeight / 2), UIBase.getUIColorTheme().edit_box_text_color_uneditable.getColorInt(), false);
        ...
        this.cancelButton = new ExtendedButton(this.width - this.borderRight - 100 - 5 - 100, this.height - 35, 100, 20, Components.translatable("fancymenu.guicomponents.cancel"), (button) -> {
            this.onClose();
        });
        this.addWidget(this.cancelButton);
        UIBase.applyDefaultWidgetSkinTo(this.cancelButton);

        this.doneButton = new ExtendedButton(this.width - this.borderRight - 100, this.height - 35, 100, 20, Components.translatable("fancymenu.guicomponents.done"), (button) -> {
        ...
        if (this.allowPlaceholders) {
            MutableComponent placeholderButtonLabel = Components.translatable("fancymenu.ui.text_editor.placeholders");
    ...
    protected void renderMultilineNotSupportedNotification(GuiGraphics graphics, int mouseX, int mouseY, float partial) {
        if (!this.multilineMode) {
            MutableComponent indicator = Components.translatable("fancymenu.editor.text_editor.single_line_warning.indicator").withStyle(Style.EMPTY.withColor(UIBase.getUIColorTheme().error_text_color.getColorInt())).append(Components.literal(" [?]").withStyle(Style.EMPTY.withBold(true).withColor(UIBase.getUIColorTheme().warning_text_color.getColorInt())));
