 ...
 * <p>
 * This class has been updated to use the modern {@link GuiGraphicsExtractor} transformation system.
 */
public class MinecraftSplashRenderer {

    // This initialization logic remains correct, as it retrieves the splash component.
    public static final MinecraftSplashRenderer DEFAULT_INSTANCE = new MinecraftSplashRenderer(((IMixinSplashRenderer)Objects.requireNonNullElse(Minecraft.getInstance().gui.splashManager().getSplash(), new SplashRenderer(Component.translatable("ERROR")))).getSplashFancyMenu());
     ...
     */
    public void extractRenderState(GuiGraphicsExtractor GuiGraphicsExtractor, int screenWidth, Font font, int color) {
        GuiGraphicsExtractor.pose().pushMatrix();

        // Use 2D transformations from the new pose stack
        GuiGraphicsExtractor.pose().translate((float)screenWidth / 2.0F + WIDTH_OFFSET, (float)HEIGHT_OFFSET);
        GuiGraphicsExtractor.pose().rotate((float) Math.toRadians(-20.0F));
        ...
        GuiGraphicsExtractor.centeredText(font, this.splash, 0, -8, 0xFFFF00 | color);
     ...
     *                    overridden to create a yellow tint, while the A and B channels are preserved.
     */
    public void renderAt(GuiGraphicsExtractor GuiGraphicsExtractor, int x, int y, Font font, int color) {
        GuiGraphicsExtractor.pose().pushMatrix();
