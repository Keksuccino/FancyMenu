...
public final class SmoothTextRenderer {
    ...
    private static void addGlyph(VertexConsumer buffer, Matrix4f matrix, SmoothFontGlyph glyph, float penX, float baseline, float scale, int color, boolean italic) {
        ...
        buffer.vertex(matrix, x0 + italicSkew, y0, 0.0F).uv(glyph.u0(), glyph.v0()).color(color).endVertex();
        buffer.vertex(matrix, x0, y1, 0.0F).uv(glyph.u0(), glyph.v1()).color(color).endVertex();
        buffer.vertex(matrix, x1, y1, 0.0F).uv(glyph.u1(), glyph.v1()).color(color).endVertex();
        buffer.vertex(matrix, x1 + italicSkew, y0, 0.0F).uv(glyph.u1(), glyph.v0()).color(color).endVertex();
    }
