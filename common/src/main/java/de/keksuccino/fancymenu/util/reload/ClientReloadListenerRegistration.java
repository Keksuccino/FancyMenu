...
final class ClientReloadListenerRegistration<L> {
    ...
    synchronized boolean register(@NotNull FancyMenuResourceReload.ClientLoader loader, @NotNull Consumer<? super L> registrar) {
        ...
        if (this.registeredLoader != null) throw new IllegalStateException("FancyMenu's client reload listener is already registered by " + this.registeredLoader + ", so " + loader + " cannot register it again");
        if (this.registeringLoader != null) throw new IllegalStateException("FancyMenu's client reload listener registration is already in progress for " + this.registeringLoader);
