...
final class MinecraftFormattingState {
    ...
    Style applyTo(@NotNull Style markdownStyle) {
        ...
        // 1.19.2's TextColor keeps every input bit, while newer versions normalize to RGB. Mask the DrawableColor's
        // ARGB alpha byte so the stored style and serialized value retain Minecraft's version-independent RGB meaning.
        if ((this.colorSource == ColorSource.CUSTOM) && (this.customColorFormatter != null)) combined = combined.withColor(this.customColorFormatter.getColor().getColorInt() & 0xFFFFFF);
