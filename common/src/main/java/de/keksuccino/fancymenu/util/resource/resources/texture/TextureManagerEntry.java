...
final class TextureManagerEntry<I extends AutoCloseable, T extends AutoCloseable> implements AutoCloseable {
    ...
    static TextureManagerEntry<NativeImage, DynamicTexture> dynamicTexture() {
        return new TextureManagerEntry<>((location, image) -> new DynamicTexture(image), (location, texture) -> Minecraft.getInstance().getTextureManager().register(location, texture), location -> Minecraft.getInstance().getTextureManager().release(location), DynamicTexture::getPixels, TextureManagerReleaseDispatcher::dispatch);
    }
    ...
    @Nullable
    ResourceLocation register(@NotNull ResourceLocation location) {
        Objects.requireNonNull(location);
        synchronized (this.lifecycleLock) {
            if (this.closed || this.registrationAttempted || (this.decodedImage == null)) return this.registeredLocation;
            this.registrationAttempted = true;
            I image = this.decodedImage;
            T texture = null;
            try {
                texture = this.textureFactory.apply(location, image);
                this.decodedImage = null;
                this.textureRegistrar.accept(location, texture);
    ...
    public void close() {
        ...
        synchronized (this.lifecycleLock) {
            ...
            location = this.registeredLocation;
            releaseRegisteredTexture = this.registered;
            this.decodedImage = null;
            this.registeredTexture = null;
            this.registeredLocation = null;
            this.registered = false;
        }
        if (image != null) CloseableUtils.closeQuietly(image);
        if ((texture == null) && (location == null)) return;
        this.releaseDispatcher.accept(() -> this.releaseRegisteredTexture(location, texture, releaseRegisteredTexture));
    }

    private void releaseRegisteredTexture(@Nullable ResourceLocation location, @Nullable T texture, boolean releaseRegisteredTexture) {
        if (releaseRegisteredTexture && (location != null)) {
            try {
                this.textureReleaser.accept(location);
                return;
            } catch (Throwable throwable) {
                LOGGER.error("[FANCYMENU] Failed to release dynamic texture-manager entry: {}", location, throwable);
            }
        }
