...
final class TextureManagerEntry<I extends AutoCloseable, T extends AutoCloseable> implements AutoCloseable {
    ...
    static TextureManagerEntry<NativeImage, DynamicTexture> dynamicTexture() {
        return new TextureManagerEntry<>((identifier, image) -> new DynamicTexture(identifier::toString, image), (identifier, texture) -> Minecraft.getInstance().getTextureManager().register(identifier, texture), identifier -> Minecraft.getInstance().getTextureManager().release(identifier), DynamicTexture::getPixels, TextureManagerReleaseDispatcher::dispatch);
    }
