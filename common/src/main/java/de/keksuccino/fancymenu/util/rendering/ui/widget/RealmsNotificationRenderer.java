...
public class RealmsNotificationRenderer {
    ...
    public void renderIcons(GuiGraphics guiGraphics, int x, int y, int color) {
        ...
        if (trialAvailable && showOldNotifications) {
            guiGraphics.blitSprite(TRIAL_AVAILABLE_SPRITE, currentX, y + 4, 8, 8);
            currentX += 16;
        }

        if (pendingInvites > 0 && showOldNotifications) {
            guiGraphics.blitSprite(INVITE_SPRITE, currentX, y + 1, 14, 14);
            currentX += 16;
        }

        if (hasUnreadNews && showOldNotifications) {
            guiGraphics.blitSprite(NEWS_SPRITE, currentX, y + 1, 14, 14);
            currentX += 16;
        }

        if (hasUnseenNotifications) {
            guiGraphics.blitSprite(UNSEEN_NOTIFICATION_SPRITE, currentX, y + 3, 10, 10);
        }
