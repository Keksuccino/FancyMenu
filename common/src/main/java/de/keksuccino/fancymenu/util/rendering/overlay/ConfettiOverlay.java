...
public class ConfettiOverlay extends AbstractWidget implements NavigatableWidget {
    ...
    protected void extractWidgetRenderState(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {
        ...
        if (deltaSeconds > 0.0F) {
            ...
            updatePieces(overlayWidth, overlayHeight, deltaSeconds);
        }
    ...
    private void renderPieces(GuiGraphicsExtractor graphics, int overlayX, int overlayY, int overlayWidth, int overlayHeight) {
    ...
    private void renderPiece(GuiGraphicsExtractor graphics, int overlayX, int overlayY, int overlayWidth, int overlayHeight, ConfettiPiece piece) {
    ...
    private boolean isPieceSupported(ConfettiPiece piece, int width, int height) {
        float bottom = piece.y + piece.height;
        float left = piece.x;
        float right = piece.x + piece.width;

        float bottomSurface = this.bottomArea != null ? this.bottomArea.y : height;
        if (isSurfaceMatch(bottom, bottomSurface) && overlapsX(left, right, 0.0F, width)) {
            return true;
        }
        ...
        for (LandingArea area : this.collisionAreas) {
            ...
            if (isSurfaceMatch(bottom, area.y)) {
                return true;
            }
        }

        for (ConfettiPiece other : this.settledPieces) {
            if (other == piece) {
                continue;
            }
            if (!overlapsX(left, right, other.x, other.x + other.width)) {
                continue;
            }
            if (isSurfaceMatch(bottom, other.y)) {
                return true;
            }
        }
    ...
    private boolean isSurfaceMatch(float bottom, float surfaceY) {
        return Math.abs(bottom - surfaceY) <= SUPPORT_EPSILON;
    }
    ...
    private int resolvePieceColor() {
        ...
        int baseAlpha = ARGB.alpha(this.baseColor);
        if (baseAlpha <= 0) {
            baseAlpha = 255;
        }
    ...
    private int updatePieceColor(int existingColor, int rgb, float previousAlphaScale, float newAlphaScale) {
        int alpha = ARGB.alpha(existingColor);
