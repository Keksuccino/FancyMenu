...
final class GlslOpenGlStateSnapshot {
    ...
    GlslOpenGlStateSnapshot(int textureUnitCount) {
        ...
        for (int i = 0; i < this.textureBindings.length; i++) {
            ...
            // A sampler object overrides the texture's filtering and wrap parameters. Raw GLSL channels intentionally
            // use their own texture parameters, so detach Minecraft's sampler until the snapshot is restored.
