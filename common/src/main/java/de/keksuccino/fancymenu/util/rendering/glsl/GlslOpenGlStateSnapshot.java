...
/** Captures every OpenGL state mutated by the GLSL runtime so the following Minecraft render pass starts unchanged. */
final class GlslOpenGlStateSnapshot {
    ...
    void restore() {
        ...
        GlStateManager._colorMask(0, this.colorWriteMask);
        ...
        GlStateManager._blendEquationSeparate(this.blendEquationRgb, this.blendEquationAlpha);
    }

    private static void setCapability(int capability, boolean enabled) {
        switch (capability) {
            case GL11.GL_BLEND -> {
                if (enabled) GlStateManager._enableBlend(0); else GlStateManager._disableBlend(0);
            }
