...
public void renderBackground(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partial) {
    ...
    protected void renderMultilineNotSupportedNotification(GuiGraphics graphics, int mouseX, int mouseY, float partial) {
        if (!this.multilineMode) {
            ...
            if (UIBase.isXYInArea(mouseX, mouseY, indicatorX, (int) indicatorY, indicatorWidth, (int) UIBase.getUITextHeightNormal())) {
                TooltipHandler.INSTANCE.addRenderTickTooltip(UITooltip.of(Component.translatable("fancymenu.editor.text_editor.single_line_warning").withStyle(Style.EMPTY.withColor(UIBase.getUITheme().error_color.getColorInt()))), () -> true);
            }
        }
    }
    ...
    protected TextEditorLine getClosestVisibleLineAtY(double mouseY) {
        int index = TextEditorSelectionMath.findClosestVisibleIndex(mouseY, this.getEditorAreaY(), this.getEditorAreaY() + this.getEditorAreaHeight(), this.textFieldLines, TextEditorLine::getY, TextEditorLine::getHeight);
        return index >= 0 ? this.textFieldLines.get(index) : null;
    }
    ...
    public boolean keyPressed(int keycode, int scancode, int modifiers) {
        ...
        if ((keycode == GLFW.GLFW_KEY_C) && InputUtils.isGuiShortcutModifierDown(modifiers)) {
            Minecraft.getInstance().keyboardHandler.setClipboard(this.getHighlightedText());
            return true;
        }
        //GUI shortcut modifier + V
        if ((keycode == GLFW.GLFW_KEY_V) && InputUtils.isGuiShortcutModifierDown(modifiers)) {
            this.history.saveSnapshot();
            this.pasteText(Minecraft.getInstance().keyboardHandler.getClipboard());
            return true;
        }
        //GUI shortcut modifier + A
        if ((keycode == GLFW.GLFW_KEY_A) && InputUtils.isGuiShortcutModifierDown(modifiers)) {
        ...
        if ((keycode == GLFW.GLFW_KEY_X) && InputUtils.isGuiShortcutModifierDown(modifiers)) {
            this.history.saveSnapshot();
            Minecraft.getInstance().keyboardHandler.setClipboard(this.cutHighlightedText());
            this.resetHighlighting();
            return true;
