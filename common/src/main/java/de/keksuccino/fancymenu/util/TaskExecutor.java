...
public class TaskExecutor {
    ...
    private static ScheduledThreadPoolExecutor createExecutor() {
        ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(20, TaskExecutor::createWorkerThread);
    ...
    private static Thread createWorkerThread(Runnable runnable) {
        Thread thread = new Thread(runnable, "FancyMenu-TaskExecutor-" + THREAD_COUNTER.incrementAndGet());
        thread.setDaemon(true);
        thread.setUncaughtExceptionHandler((failedThread, throwable) -> LOGGER.error("[FANCYMENU] Uncaught exception in task executor thread '{}'.", failedThread.getName(), throwable));
        return thread;
    }
