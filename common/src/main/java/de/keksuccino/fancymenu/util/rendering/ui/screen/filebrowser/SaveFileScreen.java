...
public class SaveFileScreen extends AbstractFileBrowserScreen {
    ...
    protected void renderFileNameEditBox(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {
        ...
        this.fileNameEditBox.extractRenderState(graphics, mouseX, mouseY, partial);
        graphics.text(this.font, FILE_NAME_PREFIX_TEXT, this.fileNameEditBox.getX() - 1 - Minecraft.getInstance().font.width(FILE_NAME_PREFIX_TEXT) - 5, this.fileNameEditBox.getY() - 1 + (this.fileNameEditBox.getHeight() / 2) - (Minecraft.getInstance().font.lineHeight / 2), UIBase.getUIColorTheme().element_label_color_normal.getColorInt(), false);
    }
