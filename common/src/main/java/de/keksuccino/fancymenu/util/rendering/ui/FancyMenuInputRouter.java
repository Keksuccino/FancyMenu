...
public final class FancyMenuInputRouter {
    ...
    public static boolean routeMouseScrolled(@NotNull Iterable<? extends GuiEventListener> listeners, double mouseX, double mouseY, double scrollDeltaY) {
        for (GuiEventListener listener : listeners) {
            if ((listener instanceof FancyMenuUiComponent) && listener.mouseScrolled(mouseX, mouseY, scrollDeltaY)) return true;
        }
        return false;
    }
