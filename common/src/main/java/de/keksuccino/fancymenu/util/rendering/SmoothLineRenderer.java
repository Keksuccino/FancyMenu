...
public final class SmoothLineRenderer {
    ...
    public static void renderSmoothHorizontalLine(@NotNull GuiGraphicsExtractor graphics, float x, float y, float width, float thickness, int color, float partial) {
        SmoothRectangleRenderer.renderSmoothRect(graphics, x, y, width, thickness, 0.0F, color, partial);
    }

    public static void renderSmoothHorizontalLineScaled(@NotNull GuiGraphicsExtractor graphics, float x, float y, float width, float thickness, int color, float partial) {
        SmoothRectangleRenderer.renderSmoothRectScaled(graphics, x, y, width, thickness, 0.0F, color, partial);
    }

    public static void renderSmoothVerticalLine(@NotNull GuiGraphicsExtractor graphics, float x, float y, float height, float thickness, int color, float partial) {
        SmoothRectangleRenderer.renderSmoothRect(graphics, x, y, thickness, height, 0.0F, color, partial);
    }

    public static void renderSmoothVerticalLineScaled(@NotNull GuiGraphicsExtractor graphics, float x, float y, float height, float thickness, int color, float partial) {
     ...
     */
    public static void renderSmoothLine(@NotNull GuiGraphicsExtractor graphics, float x1, float y1, float x2, float y2, float thickness, int color, float partial) {
        float dx = x2 - x1;
        float dy = y2 - y1;
     ...
     * If the line is not horizontal/vertical, nothing is rendered.
     */
    public static void renderSmoothLineScaled(@NotNull GuiGraphicsExtractor graphics, float x1, float y1, float x2, float y2, float thickness, int color, float partial) {
        float dx = x2 - x1;
        float dy = y2 - y1;
        if (Math.abs(dx) <= AXIS_EPSILON_FANCYMENU) {
            float startY = Math.min(y1, y2);
