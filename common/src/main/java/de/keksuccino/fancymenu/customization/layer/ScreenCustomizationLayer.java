...
public class ScreenCustomizationLayer implements ElementFactory {
	...
	public void drawToBackground(RenderedScreenBackgroundEvent e) {
		this.extractBackground(e.getGraphics(), e.getMouseX(), e.getMouseY(), e.getPartial(), e.getScreen());
	}
	...
	protected void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial, Screen screen) {
		...
		if (!this.layoutBase.menuBackgrounds.isEmpty()) {
			...
			if (this.layoutBase.applyVanillaBackgroundBlur) {
                RenderingUtils.extractVanillaLikeFullscreenBlur(graphics, screen, partial);
            }
