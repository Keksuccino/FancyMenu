...
public class SetAudioWeightScreen extends CellScreen {
    ...
    protected SetAudioWeightScreen(float preset, @NotNull Consumer<Float> callback) {
        super(Components.translatable("fancymenu.elements.audio.set_weight"));
    ...
    protected void initCells() {
        ...
        this.weightSlider = new RangeSlider(0, 0, 20, 20, Components.empty(), 0.0D, 5.0D, this.current);
        this.weightSlider.setRoundingDecimalPlace(1);
        this.weightSlider.setLabelSupplier(consumes -> Components.translatable("fancymenu.elements.audio.set_weight.weight",
                Components.literal(String.format("%.1f", this.current))));
        ...
        this.weightInfo = this.addLabelCell(Components.empty());
    ...
    public void render(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partial) {
        ...
        if (this.current == 0.0f) {
            info = Components.translatable("fancymenu.elements.audio.manage_audios.weight.zero");
        } else if (this.current < 1.0f) {
            info = Components.translatable("fancymenu.elements.audio.manage_audios.weight.low");
        } else if (this.current == 1.0f) {
            info = Components.translatable("fancymenu.elements.audio.manage_audios.weight.normal");
        } else {
            info = Components.translatable("fancymenu.elements.audio.manage_audios.weight.high");
        }
