...
public class FancyMenuPanoramaPictureInPictureRenderer implements AutoCloseable {
	...
	public void prepare(FancyMenuPanoramaRenderState renderState, GuiRenderState guiRenderState, int guiScale) {
        ...
        renderState.panoramaRenderer().renderToOutput_FancyMenu(renderState.pitch(), renderState.yaw(), width, height, this.textureView, this.depthTextureView);
	...
	private void prepareTextures(boolean needsResize, int width, int height) {
		...
		device.createCommandEncoder().clearColorAndDepthTextures(this.texture, new Vector4f(0.0F, 0.0F, 0.0F, 0.0F), this.depthTexture, RenderSystem.DEFAULT_DEPTH_CLEAR_VALUE);
	}
