...
public class ChoosePanoramaScreen extends Screen {
    ...
    protected void updatePanoramaScrollAreaContent() {
        ...
        if (this.panoramaListScrollArea.getEntries().isEmpty()) {
            this.panoramaListScrollArea.addEntry(new TextScrollAreaEntry(this.panoramaListScrollArea, Components.translatable("fancymenu.panorama.choose.no_panoramas"), (entry) -> {}));
        }
    ...
    public static class PanoramaScrollEntry extends TextListScrollAreaEntry {
        ...
        public PanoramaScrollEntry(ScrollArea parent, @NotNull String panorama, @NotNull Consumer<TextListScrollAreaEntry> onClick) {
            super(parent, Components.literal(panorama).setStyle(Style.EMPTY.withColor(UIBase.getUIColorTheme().description_area_text_color.getColorInt())), UIBase.getUIColorTheme().listing_dot_color_1.getColor(), onClick);
