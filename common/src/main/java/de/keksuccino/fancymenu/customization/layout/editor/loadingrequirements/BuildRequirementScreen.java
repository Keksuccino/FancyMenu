...
public class BuildRequirementScreen extends ModernScreen {
    ...
    public BuildRequirementScreen(@Nullable Screen parentScreen, @NotNull LoadingRequirementContainer parent, @Nullable LoadingRequirementInstance instanceToEdit, @NotNull Consumer<LoadingRequirementInstance> callback) {
        ...
        super((instanceToEdit != null) ? Components.translatable("fancymenu.editor.loading_requirement.screens.edit_requirement") : Components.translatable("fancymenu.editor.loading_requirement.screens.add_requirement"));
    ...
    protected void init() {
        ...
        this.searchBar = new ExtendedEditBox(Minecraft.getInstance().font, 20 + 1, 50 + 15 + 1, (this.width / 2) - 40 - 2, 20 - 2, Components.empty()) {
            ...
            public void renderButton(@NotNull PoseStack graphics, int mouseX, int mouseY, float partial) {
                ...
                if (this.getValue().isBlank() && !this.isFocused()) {
                    GuiGraphics.currentGraphics().drawString(this.font, Components.translatable("fancymenu.requirements.build_requirement.screen.search_requirement"), this.getX() + 4, this.getY() + (this.getHeight() / 2) - (this.font.lineHeight / 2), UIBase.getUIColorTheme().edit_box_text_color_uneditable.getColorInt(), false);
                }
            }
        };
        ...
        ExtendedButton editValueButton = new ExtendedButton(editValueButtonX, editValueButtonY, 150, 20, Components.translatable("fancymenu.editor.loading_requirement.screens.build_screen.edit_value"), (button) -> {
        ...
        ExtendedButton requirementModeButton = new ExtendedButton(requirementModeButtonX, requirementModeButtonY, 150, 20, Components.empty(), (button) -> {
            if (this.instance.mode == LoadingRequirementInstance.RequirementMode.IF) {
                ...
                .setLabelSupplier(consumes -> {
                    if (this.instance.mode == LoadingRequirementInstance.RequirementMode.IF) {
                        return Components.translatable("fancymenu.editor.loading_requirement.screens.build_screen.requirement_mode.normal");
                    }
                    return Components.translatable("fancymenu.editor.loading_requirement.screens.build_screen.requirement_mode.opposite");
                });
    ...
    public void render(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partial) {
        ...
        graphics.drawString(this.font, Components.translatable("fancymenu.editor.loading_requirement.screens.build_screen.available_requirements"), 20, 50, UIBase.getUIColorTheme().generic_text_base_color.getColorInt(), false);

        this.requirementsListScrollArea.render(graphics, mouseX, mouseY, partial);

        Component descLabel = Components.translatable("fancymenu.editor.loading_requirement.screens.build_screen.requirement_description");
    ...
    protected void setContentOfRequirementsList(@Nullable String category) {
        ...
        if (category == null) {
            ...
            //Add "Back" button
            Component backLabel = Components.translatable("fancymenu.editor.loading_requirement.screens.lists.back").withStyle(Style.EMPTY.withColor(UIBase.getUIColorTheme().warning_text_color.getColorInt()));
