...
public class LayoutEditorScreen extends Screen implements ElementFactory {
	...
	public boolean mouseDragged(MouseButtonEvent event, double $$3, double $$4) {
	    return this.mouseDragged(event.x(), event.y(), event.button(), $$3, $$4, InputUtils.getActiveModifiers());
	}
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button, double $$3, double $$4, int modifiers) {
		...
		if (this.isMouseSelection) {
			for (AbstractEditorElement<?, ?> e : this.getAllElements()) {
				...
				if (!b && InputUtils.isGuiShortcutModifierDown(modifiers)) continue; // Skip deselect if the GUI shortcut modifier is pressed
				e.setSelected(b);
			}
		}
