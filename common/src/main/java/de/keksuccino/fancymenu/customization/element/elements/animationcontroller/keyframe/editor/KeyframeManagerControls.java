    ...
    private static ExtendedEditBox createLabeledInput(@NotNull KeyframeManagerScreen screen, @NotNull String labelKey) {
        ExtendedEditBox input = new ExtendedEditBox(Minecraft.getInstance().font, (screen.width / 2) - 50, screen.stickyButton.getY() - 40, 100, 20, Component.empty()) {
            ...
            public void extractWidgetRenderState(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {
                ...
                graphics.text(Minecraft.getInstance().font, label, this.getX() + (this.getWidth() / 2) - (labelWidth / 2), this.getY() - Minecraft.getInstance().font.lineHeight - 5, UIBase.getUITheme().ui_interface_generic_text_color.getColorInt(), false);
