...
public abstract class AbstractDecorationOverlay<O extends AbstractDecorationOverlay<?>> implements Renderable, ContainerEventHandler, NarratableEntry, PropertyHolder, ContextMenuBuilder<O>, FancyMenuUiComponent {
    ...
    public final void _render(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {

        this.extractRenderState(graphics, mouseX, mouseY, partial);

        this.renderables.forEach(renderable -> renderable.extractRenderState(graphics, mouseX, mouseY, partial));
