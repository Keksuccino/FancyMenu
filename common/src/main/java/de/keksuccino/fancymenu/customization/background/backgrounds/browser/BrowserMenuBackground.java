...
public class BrowserMenuBackground extends MenuBackground<BrowserMenuBackground> {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollDeltaY) {
        return this.consumeMouseScrolled(mouseX, mouseY, scrollDeltaY);
    }
    ...
    private boolean consumeMouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.processMouseClicks.tryGetNonNullElse(true)) {
            ...
            if (button == 0 && this.attachedScreen != null) {
                this.attachedScreen.setDragging(true);
            }
        }
    ...
    private boolean consumeMouseScrolled(double mouseX, double mouseY, double scrollDeltaY) {
        if (!this.isInputCaptureActive()) {
            return false;
        }
