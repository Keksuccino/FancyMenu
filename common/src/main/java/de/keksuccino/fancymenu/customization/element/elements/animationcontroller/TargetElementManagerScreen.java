...
public class TargetElementManagerScreen extends CellScreen {
    ...
    public TargetElementManagerScreen(@NotNull AnimationControllerEditorElement editorElement, @NotNull Consumer<List<AnimationControllerElement.TargetElement>> callback) {
        super(Components.translatable("fancymenu.elements.animation_controller.manage_targets"));
    ...
    protected void initCells() {
        ...
        // Header
        this.addLabelCell(Components.translatable("fancymenu.elements.animation_controller.manage_targets.info")
        ...
        if (this.targets.isEmpty()) {
            this.addLabelCell(Components.translatable("fancymenu.elements.animation_controller.manage_targets.no_targets")
            ...
            for (AnimationControllerElement.TargetElement target : this.targets) {
                ...
                MutableComponent label = (e != null) ? e.element.getDisplayName().copy() : Components.literal("---");
                label = label.setStyle(Style.EMPTY.withColor(UIBase.getUIColorTheme().warning_text_color.getColorInt()));
                label = label.append(Components.literal(" [" + target.targetElementId + "]").setStyle(Style.EMPTY.withColor(UIBase.getUIColorTheme().generic_text_base_color.getColorInt())));
    ...
    protected void initRightSideWidgets() {
        // Add target button
        this.addRightSideButton(20, Components.translatable("fancymenu.elements.animation_controller.manage_targets.add"), button -> {
        ...
        this.addRightSideButton(20, Components.translatable("fancymenu.elements.animation_controller.manage_targets.remove"), button -> {
            ElementSelectorScreen selector = new ElementSelectorScreen(
                    this.parentLayoutEditor,
                    this,
                    this.getElementIds(),
