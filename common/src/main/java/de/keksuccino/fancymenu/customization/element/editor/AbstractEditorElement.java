...
public abstract class AbstractEditorElement<E extends AbstractEditorElement<?, ?>, N extends AbstractElement> implements Renderable, GuiEventListener, ContextMenuBuilder<E>, PropertyHolder {
    ...
    protected void renderDraggingNotAllowedOverlay(GuiGraphicsExtractor graphics) {
        if (this.renderMovingNotAllowedTime >= System.currentTimeMillis()) {
            ...
            graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, DRAGGING_NOT_ALLOWED_TEXTURE, texX, texY, 0.0F, 0.0F, texW, texH, texW, texH);
        }
    }
    ...
    protected void renderDeprecatedIndicator(GuiGraphicsExtractor graphics) {
        if (this.element.getBuilder().isDeprecated()) {
            ...
            graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, DEPRECATED_WARNING_TEXTURE, texX, texY, 0.0F, 0.0F, texW, texH, texW, texH);
    ...
    protected void renderResizeIndicators(@NotNull GuiGraphicsExtractor graphics) {
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        ...
        if (button == 0) {
            ...
            if (this.leftMouseDown && this.isGettingResized()) { // RESIZE ELEMENT
                ...
                if (resizeCorner && net.minecraft.client.Minecraft.getInstance().hasShiftDown()) {
                ...
                if (resizeHorizontal && newWidth >= 2 && (!resizeCorner || !net.minecraft.client.Minecraft.getInstance().hasShiftDown() || newHeight >= 2)) {
                    this.element.autoSizingWidth = 0;
                    this.element.autoSizingHeight = 0;
                    ...
                    if (net.minecraft.client.Minecraft.getInstance().hasShiftDown() && !resizeCorner) {
                        this.element.baseHeight = this.resizeAspectRatio.getAspectRatioHeight(this.element.baseWidth);
                    }
                }
                if (resizeVertical && newHeight >= 2 && (!resizeCorner || !net.minecraft.client.Minecraft.getInstance().hasShiftDown() || newWidth >= 2)) {
                    this.element.autoSizingWidth = 0;
                    this.element.autoSizingHeight = 0;
