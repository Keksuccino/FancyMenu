...
public class ManageAudiosScreen extends CellScreen {
    ...
    protected void initCells() {
        ...
        for (AudioElement.AudioInstance instance : this.tempAudios) {
            ...
            MutableComponent weight = Components.translatable("fancymenu.elements.audio.manage_audios.weight_display", String.format("%.1f", instance.weight)).setStyle(Style.EMPTY.withColor(UIBase.getUIColorTheme().description_area_text_color.getColorInt()));
    ...
    protected void initRightSideWidgets() {
        ...
        this.addRightSideButton(20, Components.translatable("fancymenu.elements.audio.manage_audios.set_weight"), button -> {
            RenderCell selected = this.getSelectedCell();
            if (selected != null) {
                String source = selected.getMemoryValue("source");
                if (source != null) {
