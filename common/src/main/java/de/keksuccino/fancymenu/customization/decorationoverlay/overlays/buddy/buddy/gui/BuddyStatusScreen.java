...
public class BuddyStatusScreen implements Renderable {
    ...
    public void extractRenderState(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {
        ...
        graphics.pose().translate(0, 0); // Use z=400 (same as tooltips)

        // Render background
        int borderXDiff = 35;
        int borderYDiff = 27;
        graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, textures.getStatusBackgroundTexture(), this.guiX, this.guiY, 0.0F, 0.0F, SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT);
        graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, textures.getStatusBorderTexture(), this.guiX - borderXDiff, this.guiY - borderYDiff, 0.0F, 0.0F, SCREEN_BORDER_WIDTH, SCREEN_BORDER_HEIGHT, SCREEN_BORDER_WIDTH, SCREEN_BORDER_HEIGHT);
    ...
    private void renderStatsTab(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {
        ...
        graphics.text(font, title, guiX + (SCREEN_WIDTH - font.width(title)) / 2, contentStartY, TEXT_COLOR_WHITE);
        ...
        graphics.text(font, levelText, contentStartX, contentStartY + 140, TEXT_COLOR_WHITE);
        ...
        graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, textures.getStatusIconExperienceTexture(), xpIconX, xpIconY, 0.0F, 0.0F, STATUS_ICON_SIZE, STATUS_ICON_SIZE, STATUS_ICON_SIZE, STATUS_ICON_SIZE);
    ...
    private void renderAchievementsTab(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {
        ...
        for (int i = 0; i < achievementsList.size(); i++) {
            ...
            if (isUnlocked && achievement.getExperienceReward() > 0) {
                String rewardText = I18n.get("fancymenu.buddy.achievement.reward", achievement.getExperienceReward());
                graphics.text(font, rewardText, contentStartX + listWidth - font.width(rewardText) - 5, itemY + 5, TEXT_COLOR_REWARD);
            }
        }
        ...
        if (canScrollUp) {
            graphics.text(font, "▲", guiX + SCREEN_WIDTH - 20, listStartY - 5, TEXT_COLOR_WHITE);
        }
        if (canScrollDown) {
            graphics.text(font, "▼", guiX + SCREEN_WIDTH - 20, listStartY + (ACHIEVEMENT_MAX_VISIBLE_ITEMS * ACHIEVEMENT_LIST_ITEM_HEIGHT) + 5, TEXT_COLOR_WHITE);
        }
    }
     ...
     */
    private Component renderStatusBars(GuiGraphicsExtractor graphics, int startX, int startY, int mouseX, int mouseY) {
        Component tooltip = null;
        int itemHeight = Math.max(STATUS_ICON_SIZE, STATUS_BAR_HEIGHT);
        ...
        for (int i = 0; i < STATUS_BAR_COUNT; i++) {
            ...
            graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, iconTexture, startX, iconY, 0.0F, 0.0F, STATUS_ICON_SIZE, STATUS_ICON_SIZE, STATUS_ICON_SIZE, STATUS_ICON_SIZE);
    ...
    private static void renderTexturedBar(GuiGraphicsExtractor graphics, int barX, int barY, float fillAmount, float tintR, float tintG, float tintB, Identifier backgroundTexture, Identifier barTexture) {
        ...
        if (fillWidth > 0) {
            de.keksuccino.fancymenu.util.rendering.RenderingUtils.setShaderColor(graphics, tintR, tintG, tintB, 1.0F);
            graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, barTexture, barX, barY, 0.0F, 0.0F, fillWidth, STATUS_BAR_HEIGHT, STATUS_BAR_WIDTH, STATUS_BAR_HEIGHT);
