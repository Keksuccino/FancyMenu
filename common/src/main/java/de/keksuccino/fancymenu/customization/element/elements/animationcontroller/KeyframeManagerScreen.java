...
public class KeyframeManagerScreen extends Screen {
    ...
    protected AnimationKeyframe cachedSelectedKeyframe = null;
    ...
    protected void undo() {
        ...
        LOGGER.info("################### SELECTED: " + selected); //TODO checken, warum hier immer NULL als identifier gesetzt wird ????????
        ...
        if ((selected != null) && !this.isRecording) {
            ...
            for (AnimationKeyframe f : this.workingKeyframes) {
                if (f.uniqueIdentifier.equals(selected)) {
                    frame = f;
                    LOGGER.info("############### FOUND SELECTED: " + f.uniqueIdentifier);
                    break;
                }
            }
    ...
    protected class PreviewEditorElement extends AbstractEditorElement {
        ...
        public void renderPreviewBody(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partial) {
            DrawableColor c = (KeyframeManagerScreen.this.selectedKeyframe != null) ? KEYFRAME_COLOR_SELECTED : PREVIEW_COLOR_NORMAL;
