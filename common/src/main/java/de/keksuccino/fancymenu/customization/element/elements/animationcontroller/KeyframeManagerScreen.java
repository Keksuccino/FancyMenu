...
public class KeyframeManagerScreen extends Screen {
    ...
    public KeyframeManagerScreen(LayoutEditorScreen parentEditor, AnimationControllerElement controller, Consumer<AnimationControllerMetadata> resultCallback) {
        ...
        if (!this.workingKeyframes.isEmpty()) this.applyKeyframeValuesToElement(this.workingKeyframes.getFirst(), this.previewElement);
    ...
    protected int resolveParentViewportWidth(@NotNull Window window) {
        if (this.hasInitialFramebufferSize(window)) return this.initialParentViewportWidth;
        return AnimationPreviewScalePolicy.calculateViewportDimension(window.getWidth(), this.resolveParentGuiScale(window));
    }

    protected int resolveParentViewportHeight(@NotNull Window window) {
        if (this.hasInitialFramebufferSize(window)) return this.initialParentViewportHeight;
        return AnimationPreviewScalePolicy.calculateViewportDimension(window.getHeight(), this.resolveParentGuiScale(window));
    }

    protected double resolveParentGuiScale(@NotNull Window window) {
        if (this.hasInitialFramebufferSize(window)) return this.initialParentGuiScale;
        double defaultScale = window.calculateScale(Minecraft.getInstance().options.guiScale().get(), Minecraft.getInstance().options.forceUnicodeFont().get());
        return AnimationPreviewScalePolicy.resolveParentGuiScale(window.getWidth(), window.getHeight(), defaultScale, this.parentEditor.layout.forcedScale, this.parentEditor.layout.autoScalingWidth, this.parentEditor.layout.autoScalingHeight);
    }

    protected boolean hasInitialFramebufferSize(@NotNull Window window) {
        return (window.getWidth() == this.initialFramebufferWidth) && (window.getHeight() == this.initialFramebufferHeight);
    }
    ...
    protected void applyElementValuesToKeyframe(@NotNull PreviewElement element, @NotNull AnimationKeyframe keyframe) {
        ...
        keyframe.stickyAnchor = this.isOffsetMode || element.stickyAnchor;
    }
    ...
    protected void applyKeyframeValuesToElement(@NotNull AnimationKeyframe keyframe, @NotNull PreviewElement element) {
        ...
        element.stickyAnchor = this.isOffsetMode || keyframe.stickyAnchor;
    }
