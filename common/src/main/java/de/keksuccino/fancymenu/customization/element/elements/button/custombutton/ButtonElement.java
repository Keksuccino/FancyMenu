...
public class ButtonElement extends AbstractElement implements ExecutableElement {
    ...
    public void extractRenderState(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {
        ...
        if (isEditor()) {
            net.minecraft.client.gui.components.Tooltip cachedVanillaTooltip = ((de.keksuccino.fancymenu.mixin.mixins.common.client.IMixinAbstractWidget)this.getWidget()).getTooltipHolderFancyMenu().get();
        ...
        return TEMPLATE_CACHE.resolve(layer, forSlider, () -> {
            for (AbstractElement e : layer.allElements) {
                if (e instanceof ButtonElement b) {
                    ...
                    if (b.isTemplate && b.shouldRender() && validTemplate) return b;
                }
            }
            return null;
        });
