        ...
        this.searchBar = new ExtendedEditBox(Minecraft.getInstance().font, 20 + 1, 50 + 15 + 1, (this.width / 2) - 40 - 2, 20 - 2, Component.empty()) {
            ...
            public void renderButton(@NotNull PoseStack graphics, int mouseX, int mouseY, float partial) {
                ...
                if (this.getValue().isBlank() && !this.isFocused()) {
                    GuiGraphics.currentGraphics().drawString(this.font, Component.translatable("fancymenu.actions.build_action.screen.search_action"), this.getX() + 4, this.getY() + (this.getHeight() / 2) - (this.font.lineHeight / 2), UIBase.getUIColorTheme().edit_box_text_color_uneditable.getColorInt(), false);
                }
            }
        };
