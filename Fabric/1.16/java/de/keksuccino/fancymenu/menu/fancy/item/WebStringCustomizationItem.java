	...
	public void render(MatrixStack matrix, Screen menu) throws IOException {
		...
		if (!this.updating) {
			...
			for (String s : this.lines) {
				...
				int sw = (int) (font.getWidth(s) * this.scale);
				if (w < sw) {
					w = sw;
				}
				...
				if (this.shadow) {
					font.drawWithShadow(matrix, s, x, y + (i / this.scale), Color.WHITE.getRGB());
				} else {
					font.draw(matrix, s, x, y + (i / this.scale), Color.WHITE.getRGB());
				}
				matrix.pop();
