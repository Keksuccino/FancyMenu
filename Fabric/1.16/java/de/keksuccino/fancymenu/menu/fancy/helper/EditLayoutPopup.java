...
public class EditLayoutPopup extends Popup {
	...
	public EditLayoutPopup(List<PropertiesSet> properties) {
		...
		this.loadAllButton = new AdvancedButton(0, 0, 120, 20, Locals.localize("helper.creator.editlayout.popup.loadall"), true, (press) -> {
			...
			if (!b) {
				MinecraftClient.getInstance().openScreen(new PreloadedLayoutCreatorScreen(CustomizationHelper.getInstance().current, this.fullprops));
		...
		this.loadLayoutButton = new AdvancedButton(0, 0, 120, 20, Locals.localize("helper.creator.editlayout.popup.loadselected"), true, (press) -> {
			...
			if ((s != null) && !s.equals("")) {
				...
				if (!MenuCustomization.containsCalculations(l.get(0))) {
					MinecraftClient.getInstance().openScreen(new PreloadedLayoutCreatorScreen(CustomizationHelper.getInstance().current, l));
					LayoutCreatorScreen.isActive = true;
					MenuCustomization.stopSounds();
					MenuCustomization.resetSounds();
					for (IAnimationRenderer r : AnimationHandler.getAnimations()) {
	...
	public void render(MatrixStack matrix, int mouseX, int mouseY, Screen renderIn) {
		...
		if (this.isDisplayed()) {
			...
			drawCenteredString(matrix, MinecraftClient.getInstance().textRenderer, "§l" + Locals.localize("helper.creator.editlayout"),
					renderIn.width / 2, (renderIn.height / 2) - 110, Color.WHITE.getRGB());
			
			int i = 0;
			for (String s : this.msg) {
				drawCenteredString(matrix, MinecraftClient.getInstance().textRenderer, s,
			...
			drawCenteredString(matrix, MinecraftClient.getInstance().textRenderer, Locals.localize("helper.creator.editlayout.popup.layouts"),
			...
			drawCenteredString(matrix, MinecraftClient.getInstance().textRenderer,
