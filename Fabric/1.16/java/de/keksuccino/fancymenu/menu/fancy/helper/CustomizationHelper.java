...
public class CustomizationHelper {
	...
	public static void init() {
		...
		Konkrete.getEventHandler().registerEventsFrom(instance);
	}
	...
	public void onInitPost(GuiScreenEvent.InitGuiEvent.Post e) {
		...
		AdvancedButton layoutCreatorButton = new CustomizationButton(e.getGui().width - 150, 5, 90, 20, Locals.localize("helper.button.createlayout"), true, (onPress) -> {
			MinecraftClient.getInstance().openScreen(new LayoutCreatorScreen(this.current));
		...
		AdvancedButton editLayoutButton = new CustomizationButton(e.getGui().width - 245, 5, 90, 20, Locals.localize("helper.creator.editlayout"), true, (onPress) -> {
			...
			if (l.size() == 1) {
				if (!MenuCustomization.containsCalculations(l.get(0))) {
					MinecraftClient.getInstance().openScreen(new PreloadedLayoutCreatorScreen(this.current, l));
					LayoutCreatorScreen.isActive = true;
					MenuCustomization.stopSounds();
					MenuCustomization.resetSounds();
					for (IAnimationRenderer r : AnimationHandler.getAnimations()) {
		...
		this.overrideButton = new CustomizationButton(e.getGui().width - 150, 5, 90, 20, overrLabel, true, (onPress) -> {
			if (!this.isScreenOverridden()) {
				...
				if (this.current instanceof CustomGuiBase) {
					MinecraftClient.getInstance().openScreen(((CustomGuiBase) this.current).getOverriddenScreen());
				}
			}
		});
	...
	public void onRenderPost(GuiScreenEvent.DrawScreenEvent.Post e) {
		...
		if (this.showMenuInfo && !(e.getGui() instanceof LayoutCreatorScreen)) {
			...
			DrawableHelper.fill(e.getMatrixStack(), 3, 3, 3 + w + 4, 25, this.menuinfoBackground.getRGB());
			
			e.getGui();
			DrawableHelper.drawStringWithShadow(e.getMatrixStack(), MinecraftClient.getInstance().textRenderer, infoTitle, 5, 5, 0);
			if (tick == 0) {
				e.getGui();
				DrawableHelper.drawStringWithShadow(e.getMatrixStack(), MinecraftClient.getInstance().textRenderer, "§f" + id, 5, 15, 0);
			} else {
				e.getGui();
				DrawableHelper.drawStringWithShadow(e.getMatrixStack(), MinecraftClient.getInstance().textRenderer, "§a" + Locals.localize("helper.menuinfo.idcopied"), 5, 15, 0);
			}

			int mouseX = MouseInput.getMouseX();
			int mouseY = MouseInput.getMouseY();
			if ((mouseX >= 5) && (mouseX <= 5 + w2) && (mouseY >= 15) && (mouseY <= 15 + 10) && (tick == 0)) {
				DrawableHelper.fill(e.getMatrixStack(), 5, 15 + 10 - 1, 5 + w2, 15 + 10, -1);
		...
		if (this.showButtonInfo) {
			for (ButtonData d : this.buttons) {
				if (d.getButton().isHovered()) {
					...
					RenderSystem.enableBlend();
					e.getGui();
					DrawableHelper.drawStringWithShadow(e.getMatrixStack(), MinecraftClient.getInstance().textRenderer, "§f§l" + Locals.localize("helper.button.buttoninfo"), x + 10, y + 10, 0);

					int i2 = 20;
					for (String s : info) {
						e.getGui();
						DrawableHelper.drawStringWithShadow(e.getMatrixStack(), MinecraftClient.getInstance().textRenderer, s, x + 10, y + 10 + i2, 0);
						i2 += 10;
					}
					RenderSystem.disableBlend();
	...
	private static void drawInfoBackground(MatrixStack matrix, int x, int y, int width, int height) {
		DrawableHelper.fill(matrix, x, y, x + width, y + height, new Color(102, 0, 102, 200).getRGB());
	}
	...
	private static class ManageLayoutsPopupMenu extends ContextMenu {
		...
		public void openMenuAt(AbstractButtonWidget parentBtn) {
			this.content.clear();
