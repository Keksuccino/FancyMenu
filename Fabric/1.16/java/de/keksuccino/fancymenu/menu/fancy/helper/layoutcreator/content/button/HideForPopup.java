...
public class HideForPopup extends TextInputPopup {
	...
	public HideForPopup(String title, CharacterFilter filter, int backgroundAlpha, ILayoutButton button) {
		...
		this.delayEverytimeBtn = new AdvancedButton(0, 0, 100, 20, eBtn, true, (press) -> {
			...
			press.setMessage(new LiteralText("§a" + Locals.localize("helper.creator.popup.hidefor.delayeverytime")));
		...
		this.delayOnlyFirstTimeBtn = new AdvancedButton(0, 0, 100, 20, fBtn, true, (press) -> {
			...
			press.setMessage(new LiteralText("§a" + Locals.localize("helper.creator.popup.hidefor.delayfirsttime")));
	...
	public void render(MatrixStack matrix, int mouseX, int mouseY, Screen renderIn) {
		...
		drawCenteredString(matrix, MinecraftClient.getInstance().textRenderer, title, renderIn.width / 2, (renderIn.height / 2) - (height / 2) + 10, Color.WHITE.getRGB());
