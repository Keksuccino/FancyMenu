		...
		for (PropertiesSet s : this.props) {
			...
			if (biggerthanwidth != null) {
				...
				if (MathUtils.isInteger(biggerthanwidth)) {
					...
					if (MinecraftClient.getInstance().getWindow().getWidth() < i) {
						continue;
					}
				}
			}
			...
			if (biggerthanheight != null) {
				...
				if (MathUtils.isInteger(biggerthanheight)) {
					...
					if (MinecraftClient.getInstance().getWindow().getHeight() < i) {
						continue;
					}
				}
			}
			...
			if ((biggerthan != null) && biggerthan.toLowerCase().contains("x")) {
				...
				if (MathUtils.isInteger(wRaw) && MathUtils.isInteger(hRaw)) {
					...
					if ((MinecraftClient.getInstance().getWindow().getWidth() < w) || (MinecraftClient.getInstance().getWindow().getHeight() < h)) {
						continue;
					}
				}
			}
			...
			if (this.canRenderBackground()) {
				if ((this.backgroundAnimation != null) && this.backgroundAnimation.isReady()) {
					...
					if (!this.panoramaback) {
						drawTexture(CurrentScreenHandler.getMatrixStack(), 0, 0, 1.0F, 1.0F, e.getGui().width + 1, e.getGui().height + 1, e.getGui().width + 1, e.getGui().height + 1);
					} else {
