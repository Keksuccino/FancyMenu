...
public class FancyMenuFabricClientEvents {
    ...
    public static void registerAll() {
        ...
        if (FancyMenuResourceReload.registerClientReloadListener(FancyMenuResourceReload.ClientLoader.FABRIC, listener -> ResourceLoader.get(PackType.CLIENT_RESOURCES).registerReloadListener(FancyMenuResourceReload.FANCYMENU_RELOAD_LISTENER_ID, listener))) {
    ...
    private static void registerScreenEvents() {
        ScreenEvents.BEFORE_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            ...
            ScreenKeyboardEvents.afterKeyPress(screen).register((screen1, input) -> {
                ...
                if (Minecraft.getInstance().gui.overlay() instanceof GameIntroOverlay o) o.keyPressed(key, scancode, modifiers);
            ...
            ScreenKeyboardEvents.afterKeyRelease(screen).register((screen1, input) -> {
                ...
                ScreenKeyReleasedEvent event = new ScreenKeyReleasedEvent(screen1, input);
                EventHandler.INSTANCE.postEvent(event);
