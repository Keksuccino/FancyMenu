...
public class PacketsFabric {
    ...
    private static void registerFabricBridgePacket() {
        ...
        PayloadTypeRegistry.playC2S().register(BridgeChunkPayload.TYPE, BridgeChunkPayload.CODEC);
        PayloadTypeRegistry.playS2C().register(BridgeChunkPayload.TYPE, BridgeChunkPayload.CODEC);
    ...
    private static boolean sendToServerIfNegotiated(Connection exactConnection, CustomPacketPayload payload) {
        ...
        exactConnection.send(ClientPlayNetworking.createC2SPacket(payload));
        return true;
    }
