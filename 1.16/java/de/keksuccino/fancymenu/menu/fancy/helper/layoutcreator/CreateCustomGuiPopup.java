...
public class CreateCustomGuiPopup extends Popup {
	...
	public CreateCustomGuiPopup() {
		...
		this.identifierText = new AdvancedTextField(Minecraft.getInstance().fontRenderer, 0, 0, 200, 20, true, null);
		
		this.titleText = new AdvancedTextField(Minecraft.getInstance().fontRenderer, 0, 0, 200, 20, true, null);
		
		this.allowEscButton = new AdvancedButton(0, 0, 120, 20, "§a" + Locals.localize("helper.buttons.tools.creategui.allowesc"), true, (press) -> {
			press.setMessage(new StringTextComponent("§a" + Locals.localize("helper.buttons.tools.creategui.allowesc")));
		...
		this.doNotAllowEscButton = new AdvancedButton(0, 0, 120, 20, Locals.localize("helper.buttons.tools.creategui.donotallowesc"), true, (press) -> {
			press.setMessage(new StringTextComponent("§a" + Locals.localize("helper.buttons.tools.creategui.donotallowesc")));
	...
	public void render(MatrixStack matrix, int mouseX, int mouseY, Screen renderIn) {
		...
		renderIn.drawCenteredString(matrix, font, "§l" + Locals.localize("helper.buttons.tools.creategui"), renderIn.width / 2, (renderIn.height / 2) - 110, Color.WHITE.getRGB());
		
		renderIn.drawCenteredString(matrix, font, Locals.localize("helper.buttons.tools.creategui.menuidentifier"), renderIn.width / 2, (renderIn.height / 2) - 80, Color.WHITE.getRGB());
		...
		renderIn.drawCenteredString(matrix, font, Locals.localize("helper.buttons.tools.creategui.menutitle"), renderIn.width / 2, (renderIn.height / 2) - 37, Color.WHITE.getRGB());
		...
		//---------
		renderIn.drawCenteredString(matrix, font, Locals.localize("helper.buttons.tools.creategui.allowescdesc"), renderIn.width / 2, (renderIn.height / 2) + 5, Color.WHITE.getRGB());
		...
		if ((id != null) && (id.length() > 0) && !id.contains(" ")) {
			...
			if (!l.contains(id)) {
				...
				renderIn.drawCenteredString(matrix, font, Locals.localize("helper.buttons.tools.creategui.identifieralreadyused"), renderIn.width / 2, (renderIn.height / 2) + 85, Color.WHITE.getRGB());
			}
		} else {
			this.doneButton.visible = false;
			renderIn.drawCenteredString(matrix, font, Locals.localize("helper.buttons.tools.creategui.invalididentifier"), renderIn.width / 2, (renderIn.height / 2) + 85, Color.WHITE.getRGB());
		}
	...
	private void onDoneButtonPressed() {
		try {
			...
			if (this.identifierText.getText() != null) {
				...
				CustomGuiBase gui = CustomGuiLoader.getGui(this.identifierText.getText(), Minecraft.getInstance().currentScreen, null);
				if (gui != null) {
					Minecraft.getInstance().displayGuiScreen(gui);
					Minecraft.getInstance().displayGuiScreen(new LayoutCreatorScreen(gui));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
