...
public class ExcludeMenuPopup extends TextInputPopup {
	...
	protected void init(Color color, String title, CharacterFilter filter, Consumer<String> callback) {
		...
		this.noBundleExcludeButton = new AdvancedButton(0, 0, 120, 20, "§a" + Locals.localize("helper.excludemenu.excludejustthis"), true, (press) -> {
			this.bundle = false;
			this.bundleExcludeButton.setMessage(new StringTextComponent(Locals.localize("helper.excludemenu.excludeall")));
			press.setMessage(new StringTextComponent("§a" + Locals.localize("helper.excludemenu.excludejustthis")));
		});
		...
		this.bundleExcludeButton = new AdvancedButton(0, 0, 120, 20, Locals.localize("helper.excludemenu.excludeall"), true, (press) -> {
			this.bundle = true;
			this.noBundleExcludeButton.setMessage(new StringTextComponent(Locals.localize("helper.excludemenu.excludejustthis")));
			press.setMessage(new StringTextComponent("§a" + Locals.localize("helper.excludemenu.excludeall")));
		});
	...
	public void render(MatrixStack matrix, int mouseX, int mouseY, Screen renderIn) {
		if (this.isDisplayed()) {
			...
			drawCenteredString(matrix, Minecraft.getInstance().fontRenderer, new StringTextComponent("§l" + Locals.localize("helper.excludemenu.byidentifier")), renderIn.width / 2, (renderIn.height / 2) - (this.textField.func_238483_d_() / 2) - 65, Color.WHITE.getRGB());
			
			drawCenteredString(matrix, Minecraft.getInstance().fontRenderer, new StringTextComponent(Locals.localize("helper.menuinfo.identifier")), renderIn.width / 2, (renderIn.height / 2) - (this.textField.func_238483_d_() / 2) - 15, Color.WHITE.getRGB());
			...
			drawCenteredString(matrix, Minecraft.getInstance().fontRenderer, new StringTextComponent(Locals.localize("helper.excludemenu.excludebundle")), renderIn.width / 2, (renderIn.height / 2) + 25, Color.WHITE.getRGB());
