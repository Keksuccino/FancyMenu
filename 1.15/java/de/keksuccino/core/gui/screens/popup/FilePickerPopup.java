...
public class FilePickerPopup extends Popup {
	...
	public void render(int mouseX, int mouseY, Screen renderIn) {
		...
		fill(0, 0, renderIn.width, 40, this.overlayColor.getRGB());
		fill(0, renderIn.height - 60, renderIn.width, renderIn.height, this.overlayColor.getRGB());
		
		drawCenteredString(Minecraft.getInstance().fontRenderer, "§l" + Locals.localize("popup.choosefile.title"), renderIn.width / 2, 17, Color.WHITE.getRGB());
	...
	public static class FileChooserEntry extends ScrollAreaEntry {
		...
		public void renderEntry() {
			...
			blit(this.x, this.y, 0.0F, 0.0F, 20, 20, 20, 20);
			
			if (this.type == Type.BACK) {
				Minecraft.getInstance().fontRenderer.drawStringWithShadow(Locals.localize("popup.choosefile.back"), this.x + 30, this.y + 7, Color.WHITE.getRGB());
			} else {
				Minecraft.getInstance().fontRenderer.drawStringWithShadow(this.file.getName(), this.x + 30, this.y + 7, Color.WHITE.getRGB());
			}
		...
		private void renderBorder() {
			//left
			fill(this.x, this.y, this.x + 1, this.y + this.getHeight(), Color.WHITE.getRGB());
			//right
			fill(this.x + this.getWidth() - 1, this.y, this.x + this.getWidth(), this.y + this.getHeight(), Color.WHITE.getRGB());
			//top
			fill(this.x, this.y, this.x + this.getWidth(), this.y + 1, Color.WHITE.getRGB());
			//bottom
			fill(this.x, this.y + this.getHeight() - 1, this.x + this.getWidth(), this.y + this.getHeight(), Color.WHITE.getRGB());
		}
