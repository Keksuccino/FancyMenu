...
public class WindowSizePopup extends FMPopup {
	...
	public void render(PoseStack matrix, int mouseX, int mouseY, Screen renderIn) {
		...
		if (this.type == ActionType.BIGGERTHAN) {
			GuiComponent.drawCenteredString(matrix, font, "§l" + Locals.localize("helper.creator.windowsize.biggerthan.desc"), renderIn.width / 2, (renderIn.height / 2) - 110, Color.WHITE.getRGB());
		} else {
			GuiComponent.drawCenteredString(matrix, font, "§l" + Locals.localize("helper.creator.windowsize.smallerthan.desc"), renderIn.width / 2, (renderIn.height / 2) - 110, Color.WHITE.getRGB());
		}
		
		GuiComponent.drawCenteredString(matrix, font, Locals.localize("general.width"), renderIn.width / 2, (renderIn.height / 2) - 80, Color.WHITE.getRGB());
		...
		GuiComponent.drawCenteredString(matrix, font, Locals.localize("general.height"), renderIn.width / 2, (renderIn.height / 2) - 37, Color.WHITE.getRGB());
		...
		GuiComponent.drawCenteredString(matrix, font, Locals.localize("helper.creator.windowsize.currentwidth") + ": " + Minecraft.getInstance().getWindow().getScreenWidth(), renderIn.width / 2, (renderIn.height / 2) + 15, Color.WHITE.getRGB());
		GuiComponent.drawCenteredString(matrix, font, Locals.localize("helper.creator.windowsize.currentheight") + ": " + Minecraft.getInstance().getWindow().getScreenHeight(), renderIn.width / 2, (renderIn.height / 2) + 30, Color.WHITE.getRGB());
