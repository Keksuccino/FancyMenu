...
public class TitleScreenSplashItemAbstract extends AbstractDeepElement {
    ...
    public TitleScreenSplashItemAbstract(DeepElementBuilder parentElement, PropertyContainer item) {

        super(parentElement, item);

        this.splashTextFilePath = item.getValue("splash_file_path");

        String splashRot = item.getValue("splash_rotation");
        if ((splashRot != null) && MathUtils.isInteger(splashRot)) {
            splashRotation = Integer.parseInt(splashRot);
        }

        String splashCol = item.getValue("splash_color");
        if (splashCol != null) {
