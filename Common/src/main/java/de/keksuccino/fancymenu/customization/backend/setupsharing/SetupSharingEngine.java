package de.keksuccino.fancymenu.customization.backend.setupsharing;
...
import de.keksuccino.fancymenu.customization.backend.animation.ResourcePackAnimationRenderer;
...
public class SetupSharingEngine {
    ...
    protected static boolean generateUnableToExportLog(List<String> list, File exportTo) {
        try {
            ...
            MenuCustomization.openFile(exportFailsFile);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected static void displayExportSuccessPopup(File exportTo) {
        FMYesNoPopup pop = new FMYesNoPopup(300, new Color(0, 0, 0, 0), 240, (call2) -> {
            try {
                if (call2) {
                    MenuCustomization.openFile(exportTo);
                }
            } catch (Exception e) {
                e.printStackTrace();
                displayExportErrorPopup();
