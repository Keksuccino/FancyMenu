...
public class ChooseSavedSetupPopup extends FMPopup {
    ...
    public void render(PoseStack matrix, int mouseX, int mouseY, Screen renderIn) {
        ...
        if (hoveredEntry != null) {
            ...
            if (props.isFile()) {
                SetupSharingHandler.SetupProperties sp = SetupSharingHandler.deserializePropertiesFile(props.getPath());
                String modified = "???";
                try {
    ...
    public void updateScrollList() {
        ...
        File f = SetupSharingHandler.FM_SETUPS_DIR;
        if (f.isDirectory()) {
            for (File setupDir : Objects.requireNonNull(f.listFiles())) {
                if (SetupSharingHandler.isValidSetup(setupDir.getPath())) {
