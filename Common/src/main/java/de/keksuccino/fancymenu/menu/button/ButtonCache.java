	...
	private static void cache(Screen s) {
		if (!cached) {
			...
			if (!MenuCustomization.isMenuCustomizable(s)) {
				replaced.clear();
				buttons.clear();
				EventHandler.INSTANCE.postEvent(new ButtonCachedEvent(s, new ArrayList<ButtonData>(), false));
				return;
			}
			...
			EventHandler.INSTANCE.postEvent(new ButtonCachedEvent(s, getButtons(), true));
		}
	}
