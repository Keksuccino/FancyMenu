...
public class ScrollArea extends UIBase implements GuiEventListener, Renderable, NarratableEntry {
    ...
    public Supplier<DrawableColor> backgroundColor = () -> area_background_color;
    public Supplier<DrawableColor> borderColor = () -> element_border_color_normal;
    ...
    public ScrollArea(float x, float y, float width, float height) {
        ...
        this.verticalScrollBar = new ScrollBar(ScrollBar.ScrollBarDirection.VERTICAL, VERTICAL_SCROLL_BAR_WIDTH, VERTICAL_SCROLL_BAR_HEIGHT, 0, 0, 0, 0, () -> scroll_grabber_color_normal, () -> scroll_grabber_color_hover);
        this.verticalScrollBar.setScrollWheelAllowed(true);
        this.horizontalScrollBar = new ScrollBar(ScrollBar.ScrollBarDirection.HORIZONTAL, HORIZONTAL_SCROLL_BAR_WIDTH, HORIZONTAL_SCROLL_BAR_HEIGHT, 0, 0, 0, 0, () -> scroll_grabber_color_normal, () -> scroll_grabber_color_hover);
    ...
    public void renderBackground(GuiGraphics graphics, int mouseX, int mouseY, float partial) {
        fillF(graphics.pose(), this.getInnerX(), this.getInnerY(), this.getInnerX() + this.getInnerWidth(), this.getInnerY() + this.getInnerHeight(), this.backgroundColor.get().getColorInt());
    }

    public void renderBorder(GuiGraphics graphics, int mouseX, int mouseY, float partial) {
        renderBorder(graphics.pose(), this.getXWithBorder(), this.getYWithBorder(), this.getXWithBorder() + this.getWidthWithBorder(), this.getYWithBorder() + this.getHeightWithBorder(), this.getBorderThickness(), this.borderColor.get().getColorInt(), true, true, true, true);
    }
...
//            RenderSystem.enableScissor((int)(this.getInnerX() * scale), (int)(win.getHeight() - (sciBottomY * scale)), (int)(this.getInnerWidth() * scale), (int)(this.getInnerHeight() * scale));
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double deltaX, double deltaY) {
        if (this.verticalScrollBar.mouseScrolled(mouseX, mouseY, deltaX, deltaY)) return true;
        if (this.horizontalScrollBar.mouseScrolled(mouseX, mouseY, deltaX, deltaY)) return true;
        return false;
    }
