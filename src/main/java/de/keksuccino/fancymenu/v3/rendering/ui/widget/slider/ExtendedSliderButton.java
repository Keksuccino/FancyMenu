...
public abstract class ExtendedSliderButton extends AbstractSliderButton implements UniqueWidget {
    ...
    protected void renderBackground(@NotNull GuiGraphics graphics) {
        ...
        if (this.backgroundColor == null) {
            ...
            if (this.borderColor != null) {
                UIBase.renderBorder(graphics.pose(), this.getX(), this.getY(), this.getX() + this.getWidth(), this.getY() + this.getHeight(), 1, this.borderColor, true, true, true, true);
            }
        }
        RenderingUtils.resetShaderColor();
    }
    ...
    private ResourceLocation getSprite() {
        return this.isFocused() && !this.getAccessor().getCanChangeValueFancyMenu() ? HIGHLIGHTED_SPRITE : SLIDER_SPRITE;
    }

    private ResourceLocation getHandleSprite() {
        return !this.isHovered && !this.getAccessor().getCanChangeValueFancyMenu() ? SLIDER_HANDLE_SPRITE : SLIDER_HANDLE_HIGHLIGHTED_SPRITE;
    }
