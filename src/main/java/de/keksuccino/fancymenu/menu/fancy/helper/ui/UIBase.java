...
public class UIBase extends AbstractGui {
	...
	public static void renderListingDot(int x, int y, Color color) {
		drawRect(x, y, x + 4, y + 4, color.getRGB());
	}

	public static void renderBorder(int xMin, int yMin, int xMax, int yMax, int borderThickness, Color borderColor, boolean renderTop, boolean renderLeft, boolean renderRight, boolean renderBottom) {
		if (renderTop) {
			drawRect(xMin, yMin, xMax, yMin + borderThickness, borderColor.getRGB());
		}
		if (renderLeft) {
			drawRect(xMin, yMin + borderThickness, xMin + borderThickness, yMax - borderThickness, borderColor.getRGB());
		}
		if (renderRight) {
			drawRect(xMax - borderThickness, yMin + borderThickness, xMax, yMax - borderThickness, borderColor.getRGB());
		}
		if (renderBottom) {
			drawRect(xMin, yMax - borderThickness, xMax, yMax, borderColor.getRGB());
		}
	}
