...
public class LayoutSplashText extends LayoutElement {
	...
	public void init() {
		...
		AdvancedButton bounceButton = new AdvancedButton(0, 0, 0, 0, bounceLabel, true, (press) -> {
			if (this.getObject().bounce) {
				((AdvancedButton)press).displayString = Locals.localize("helper.creator.items.splash.bounce.off");
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
				
				this.getObject().bounce = false;
			} else {
				((AdvancedButton)press).displayString = Locals.localize("helper.creator.items.splash.bounce.on");
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
		...
		AdvancedButton refreshButton = new AdvancedButton(0, 0, 0, 0, refreshLabel, true, (press) -> {
			if (this.getObject().refreshOnMenuReload) {
				((AdvancedButton)press).displayString = Locals.localize("helper.creator.items.splash.refresh.off");
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
				
				this.getObject().refreshOnMenuReload = false;
			} else {
				((AdvancedButton)press).displayString = Locals.localize("helper.creator.items.splash.refresh.on");
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
	...
	protected void renderBorder(int mouseX, int mouseY) {
		//horizontal line top
		drawRect(this.object.getPosX(handler), this.object.getPosY(handler), this.object.getPosX(handler) + this.object.width, this.object.getPosY(handler) + 1, Color.BLUE.getRGB());
		//horizontal line bottom
		drawRect(this.object.getPosX(handler), this.object.getPosY(handler) + this.object.height - 1, this.object.getPosX(handler) + this.object.width, this.object.getPosY(handler) + this.object.height, Color.BLUE.getRGB());
		//vertical line left
		drawRect(this.object.getPosX(handler), this.object.getPosY(handler), this.object.getPosX(handler) + 1, this.object.getPosY(handler) + this.object.height, Color.BLUE.getRGB());
		//vertical line right
		drawRect(this.object.getPosX(handler) + this.object.width - 1, this.object.getPosY(handler), this.object.getPosX(handler) + this.object.width, this.object.getPosY(handler) + this.object.height, Color.BLUE.getRGB());
		
		//Render pos and size values
		RenderUtils.setScale(0.5F);
		drawString(Minecraft.getMinecraft().fontRenderer, Locals.localize("helper.creator.items.border.orientation") + ": " + this.object.orientation, this.object.getPosX(handler)*2, (this.object.getPosY(handler)*2) - 26, Color.WHITE.getRGB());
		drawString(Minecraft.getMinecraft().fontRenderer, Locals.localize("helper.creator.items.border.posx") + ": " + this.object.getPosX(handler), this.object.getPosX(handler)*2, (this.object.getPosY(handler)*2) - 17, Color.WHITE.getRGB());
		drawString(Minecraft.getMinecraft().fontRenderer, Locals.localize("helper.creator.items.border.width") + ": " + this.object.width, this.object.getPosX(handler)*2, (this.object.getPosY(handler)*2) - 8, Color.WHITE.getRGB());
		
		drawString(Minecraft.getMinecraft().fontRenderer, Locals.localize("helper.creator.items.border.posy") + ": " + this.object.getPosY(handler), ((this.object.getPosX(handler) + this.object.width)*2)+3, ((this.object.getPosY(handler) + this.object.height)*2) - 14, Color.WHITE.getRGB());
		drawString(Minecraft.getMinecraft().fontRenderer, Locals.localize("helper.creator.items.border.height") + ": " + this.object.height, ((this.object.getPosX(handler) + this.object.width)*2)+3, ((this.object.getPosY(handler) + this.object.height)*2) - 5, Color.WHITE.getRGB());
