    ...
    public void loadTexture() {
        if (!this.loaded) {
            try {
                ...
                this.location = MinecraftClient.getInstance().getTextureManager().registerDynamicTexture("externaltexture", new SelfcleaningDynamicTexture(i));
                this.in.close();
                this.loaded = true;
    ...
    protected static void copyPixelArea(NativeImage in, int xFrom, int yFrom, int xTo, int yTo, int width, int height, boolean mirrorX) {
        ...
        while (vertical < yTo + height) {
            ...
            while(horizontal < xTo + width) {
                int pixel = in.getPixelColor(xFrom + horiOffset, yFrom + vertOffset);
                in.setPixelColor(horizontal, vertical, pixel);
                horizontal++;
                if (mirrorX) {
                    horiOffset--;
                } else {
