...
public class SetupSharingEngine {
    ...
    public static List<String> exportSetupRaw(String baseDir, String setupName) {
        ...
        try {
            ...
            File exportToTemp = new File(Minecraft.getMinecraft().mcDataDir, "fm_export_temp_folder_" + UUID.randomUUID());
    ...
    protected static void writeSetupPropertiesFile(String saveToPathWithFileName) {
        try {
            ...
            meta.addEntry("mcversion", ForgeVersion.mcVersion);
        ...
        if (AnimationHandler.animationExists(advancedAnimationName)) {
            ...
            if (a instanceof AdvancedAnimation) {
                ...
                if (mainAni instanceof ResourcePackAnimationRenderer) {
                    ...
                    File lmrAni = new File(Minecraft.getMinecraft().mcDataDir, "resources/" + namespace);
                    if (lmrAni.isDirectory()) {
                        meta.type = AnimationType.LMR;
                        meta.resourcesPath = lmrAni.getPath();
                        return meta;
                    }
                    File resPackDir = new File(Minecraft.getMinecraft().mcDataDir, "resourcepacks");
                            ...
                            if (doStep) {
                                ...
                                if (step == 2) {
                                    if (setupProperties.mcVersion != null) {
                                        if (!setupProperties.mcVersion.equals(ForgeVersion.mcVersion)) {
                                            doStep = false;
                                            FMYesNoPopup pop = new FMYesNoPopup(300, new Color(0, 0, 0, 0), 240, (call) -> {
                                                if (call) {
                                                    step = 3;
                                            ...
                                            }, StringUtils.splitLines(Locals.localize("fancymenu.helper.setupsharing.import.differentmcversion", setupProperties.mcVersion, ForgeVersion.mcVersion), "%n%"));
                                            PopupHandler.displayPopup(pop);
                                        } else {
                                            step = 3;
                                        }
                                ...
                                if (step == 6) {
                                    try {
                                        ...
                                        if (this.setupInstancePath.isDirectory()) {
                                            File targetRaw = Minecraft.getMinecraft().mcDataDir;
    ...
    public static class StatusPopup extends FMPopup {
        ...
        public void render(int mouseX, int mouseY, GuiScreen renderIn) {
            ...
            drawCenteredString(Minecraft.getMinecraft().fontRenderer, this.status, centerX, centerY, -1);
        }
