...
public class CustomizationHelperUI extends UIBase {
	...
	protected static void renderButtonInfo(Screen screen) {
		if (showButtonInfo) {
			for (ButtonData d : buttons) {
				if (d.getButton().isHovered()) {
					...
					RenderSystem.scalef(getUIScale(), getUIScale(), getUIScale());
					...
					fill(x, y, x + width + 10, y + 80, new Color(102, 0, 102, 200).getRGB());
					
					RenderSystem.enableBlend();
					screen.drawString(Minecraft.getInstance().fontRenderer, "§f§l" + Locals.localize("helper.button.buttoninfo"), x + 10, y + 10, 0);

					int i2 = 20;
					for (String s : info) {
						screen.drawString(Minecraft.getInstance().fontRenderer, s, x + 10, y + 10 + i2, 0);
						i2 += 10;
					}
	...
	protected static void renderMenuInfo(Screen screen) {
		if (showMenuInfo) {
			...
			fill(3, h, 3 + w + 4, h + 23, new Color(0, 0, 0, 240).getRGB());

			screen.drawString(Minecraft.getInstance().fontRenderer, infoTitle, 5, h + 2, 0);
			if (tick == 0) {
				screen.drawString(Minecraft.getInstance().fontRenderer, "§f" + id, 5, h + 13, 0);
			} else {
				screen.drawString(Minecraft.getInstance().fontRenderer, "§a" + Locals.localize("helper.menuinfo.idcopied"), 5, h + 13, 0);
			}
			...
			if (!bar.isChildOpen()) {
				if ((mouseX >= 5) && (mouseX <= 5 + w2) && (mouseY >= h + 13) && (mouseY <= h + 13 + 10) && (tick == 0)) {
					fill(5, h + 13 + 10 - 1, 5 + w2, h + 13 + 10, -1);
	...
	protected static void renderUnicodeWarning(Screen screen) {
		if (Minecraft.getInstance().gameSettings.forceUnicodeFont) {
			...
			fill(x - 4, y, x + w + 2, y + h, new Color(230, 15, 0, 240).getRGB());

			screen.drawString(Minecraft.getInstance().fontRenderer, title, x, y + 2, Color.WHITE.getRGB());
			
			int i = 0;
			for (String s : lines) {
				screen.drawString(Minecraft.getInstance().fontRenderer, s, x, y + 13 + i, Color.WHITE.getRGB());
				i += 10;
			}
