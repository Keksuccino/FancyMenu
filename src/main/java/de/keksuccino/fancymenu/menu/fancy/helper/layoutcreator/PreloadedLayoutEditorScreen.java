					...
					if (action.equalsIgnoreCase("hidebuttonfor")) {
						if (b != null) {
							...
							if ((seconds != null) && (MathUtils.isFloat(seconds))) {
								...
								vanillas.get(b.getId()).object.delayAppearance = true;
								
								float ds = Float.parseFloat(seconds);
								vanillas.get(b.getId()).object.delayAppearanceSec = ds;
								this.vanillaDelayAppearance.put(b.getId(), ds);

								if ((firsttime != null) && firsttime.equalsIgnoreCase("true")) {
									vanillas.get(b.getId()).object.delayAppearanceEverytime = false;
									this.vanillaDelayAppearanceFirstTime.put(b.getId(), true);
								} else {
									vanillas.get(b.getId()).object.delayAppearanceEverytime = true;
								...
								if ((fade != null && fade.equalsIgnoreCase("true"))) {
									...
									vanillas.get(b.getId()).object.fadeIn = true;
									vanillas.get(b.getId()).object.fadeInSpeed = fs;
