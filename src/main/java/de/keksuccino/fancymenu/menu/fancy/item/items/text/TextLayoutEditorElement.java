...
//        this.resizeableX = false;
        ...
        AdvancedButton toggleSourceModeButton = new AdvancedButton(0, 0, 0, 0, "", (press) -> {
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (i.sourceMode == TextCustomizationItem.SourceMode.LOCAL_SOURCE) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.source_mode.mode.local");
                }
                if (i.sourceMode == TextCustomizationItem.SourceMode.WEB_SOURCE) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.source_mode.mode.web");
                }
                if (i.sourceMode == TextCustomizationItem.SourceMode.DIRECT) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.source_mode.mode.direct");
                }
        ...
        AdvancedButton setSourceButton = new AdvancedButton(0, 0, 0, 0, "", (press) -> {
            if (i.sourceMode == TextCustomizationItem.SourceMode.LOCAL_SOURCE) {
                ...
                if (i.source != null) {
                    ...
                    if (!f.exists() || !f.getAbsolutePath().replace("\\", "/").startsWith(Minecraft.getMinecraft().mcDataDir.getAbsolutePath().replace("\\", "/"))) {
                        f = new File(Minecraft.getMinecraft().mcDataDir.getAbsolutePath().replace("\\", "/") + "/" + i.source);
                    }
                    if (f.isFile()) {
                        p.setText(i.source);
                    }
            ...
            if ((i.sourceMode == TextCustomizationItem.SourceMode.DIRECT) || (i.sourceMode == TextCustomizationItem.SourceMode.WEB_SOURCE)) {
                TextEditorScreen s = new TextEditorScreen(Component.literal(press.displayString), this.handler, null, (call) -> {
                    if (call != null) {
                        call = call.replace("\n", "%n%");
                        if (call.length() == 0) {
                            if (i.source != null) {
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (i.sourceMode == TextCustomizationItem.SourceMode.LOCAL_SOURCE) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.set_source.local");
                    this.setDescription(StringUtils.splitLines(Locals.localize("fancymenu.customization.items.text.set_source.local.desc"), "%n%"));
                }
                if (i.sourceMode == TextCustomizationItem.SourceMode.WEB_SOURCE) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.set_source.web");
                    this.setDescription(StringUtils.splitLines(Locals.localize("fancymenu.customization.items.text.set_source.web.desc"), "%n%"));
                }
                if (i.sourceMode == TextCustomizationItem.SourceMode.DIRECT) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.set_source.direct");
        ...
        AdvancedButton caseModeButton = new AdvancedButton(0, 0, 0, 0, "", (press) -> {
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (i.caseMode == TextCustomizationItem.CaseMode.NORMAL) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.case_mode.normal");
                }
                if (i.caseMode == TextCustomizationItem.CaseMode.ALL_LOWER) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.case_mode.lower");
                }
                if (i.caseMode == TextCustomizationItem.CaseMode.ALL_UPPER) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.case_mode.upper");
                }
                super.drawButton(mc, mouseX, mouseY, partialTicks);
            }
        };
        ...
        AdvancedButton shadowButton = new AdvancedButton(0, 0, 0, 0, "", (press) -> {
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (i.shadow) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.shadow.on");
                } else {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.shadow.off");
                }
                super.drawButton(mc, mouseX, mouseY, partialTicks);
            }
        };
        ...
        AdvancedButton alignmentButton = new AdvancedButton(0, 0, 0, 0, "", (press) -> {
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (i.alignment == CustomizationItemBase.Alignment.LEFT) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.alignment.left");
                }
                if (i.alignment == CustomizationItemBase.Alignment.CENTERED) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.alignment.center");
                }
                if (i.alignment == CustomizationItemBase.Alignment.RIGHT) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.alignment.right");
                }
                super.drawButton(mc, mouseX, mouseY, partialTicks);
            }
        };
        ...
        AdvancedButton scrollingButton = new AdvancedButton(0, 0, 0, 0, "", (press) -> {
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (i.enableScrolling) {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.scrolling.on");
                } else {
                    this.displayString = Locals.localize("fancymenu.customization.items.text.scrolling.off");
                }
                super.drawButton(mc, mouseX, mouseY, partialTicks);
            }
        };
