...
public class TextEditorLine extends AdvancedTextField {
    ...
    protected IFormattableTextComponent getFormattedText(String text) {
        ...
        for (char c : text.toCharArray()) {
            ...
            chars.add(new StringTextComponent(String.valueOf(c)).withStyle(style));
...
//                this.font2.draw(matrix, this.getAsAccessor().getFormatterFancyMenu().apply(text.substring(cursorPos), this.getCursorPosition()), (float)textXRender, (float)textY, textColorInt);
            ...
            if (renderCursor) {
                if (isCursorAtEndOfLine) {
                    AbstractGui.fill(matrix, cursorPosRender, textY - 1, cursorPosRender + 1, textY + 1 + 9, -3092272);
                } else {
