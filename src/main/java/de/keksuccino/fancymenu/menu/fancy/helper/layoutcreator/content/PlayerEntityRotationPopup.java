...
public class PlayerEntityRotationPopup extends FMPopup {
	...
	public void render(MatrixStack matrix, int mouseX, int mouseY, Screen renderIn) {
		...
		drawCenteredText(matrix, font, Locals.localize("helper.creator.items.playerentity.rotation.custom.bodyx"), midX, midY - 83, Color.WHITE.getRGB());
		
		this.bodyX.x = midX - (this.bodyX.getWidth() / 2);
		this.bodyX.y = midY - 70;
		this.bodyX.render(matrix, mouseX, mouseY, partial);
		
		drawCenteredText(matrix, font, Locals.localize("helper.creator.items.playerentity.rotation.custom.bodyy"), midX, midY - 43, Color.WHITE.getRGB());
		
		this.bodyY.x = midX - (this.bodyY.getWidth() / 2);
		this.bodyY.y = midY - 30;
		this.bodyY.render(matrix, mouseX, mouseY, partial);

		drawCenteredText(matrix, font, Locals.localize("helper.creator.items.playerentity.rotation.custom.headx"), midX, midY - 3, Color.WHITE.getRGB());
		
		this.headX.x = midX - (this.headX.getWidth() / 2);
		this.headX.y = midY + 10;
		this.headX.render(matrix, mouseX, mouseY, partial);

		drawCenteredText(matrix, font, Locals.localize("helper.creator.items.playerentity.rotation.custom.heady"), midX, midY + 37, Color.WHITE.getRGB());
