	...
	public void render(MatrixStack matrix, Screen menu) throws IOException {
		if (this.shouldRender()) {
			...
			if (this.isTextureReady()) {
				...
				if (this.ready) {
					drawCenteredText(matrix, MinecraftClient.getInstance().textRenderer, "§lMISSING", this.getPosX(menu) + (this.width / 2), this.getPosY(menu) + (this.height / 2) - (MinecraftClient.getInstance().textRenderer.fontHeight / 2), -1);
				}
			}

			if (!this.ready && isEditorActive()) {
				drawCenteredText(matrix, MinecraftClient.getInstance().textRenderer, "§lLOADING TEXTURE..", this.getPosX(menu) + (this.width / 2), this.getPosY(menu) + (this.height / 2) - (MinecraftClient.getInstance().textRenderer.fontHeight / 2), -1);
			}
