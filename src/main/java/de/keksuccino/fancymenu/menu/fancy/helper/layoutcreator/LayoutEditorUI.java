	...
	public void updateUI() {
		try {
			...
			AdvancedButton newLayoutButton = new AdvancedButton(0, 0, 0, 0, Locals.localize("helper.editor.ui.layout.new"), true, (press) -> {
				this.displayUnsavedWarning((call) -> {
					if (call) {
						...
						Minecraft.getMinecraft().displayGuiScreen(new LayoutEditorScreen(this.parent.screen));
					}
				});
			});
			...
			CustomizationButton layoutTab = new CustomizationButton(0, 0, 0, 0, Locals.localize("helper.editor.ui.layout"), true, (press) -> {
				...
				layoutMenu.openMenuAt(press.x, press.y + press.height);
			});
			...
			AdvancedButton undoButton = new AdvancedButton(0, 0, 0, 0, Locals.localize("helper.editor.ui.edit.undo"), true, (press) -> {
				...
				try {
					((LayoutEditorScreen)Minecraft.getMinecraft().currentScreen).ui.bar.getChild("fm.editor.ui.tab.edit").openMenuAt(editMenu.getX(), editMenu.getY());
				} catch (Exception e) {
					e.printStackTrace();
				}
			});
			...
			CustomizationButton editTab = new CustomizationButton(0, 0, 0, 0, Locals.localize("helper.editor.ui.edit"), true, (press) -> {
				...
				editMenu.openMenuAt(press.x, press.y + press.height);
			});
			...
			CustomizationButton elementTab = new CustomizationButton(0, 0, 0, 0, Locals.localize("helper.editor.ui.element"), true, (press) -> {
				...
				elementMenu.openMenuAt(press.x, press.y + press.height);
			});
	...
	public void closeEditor() {
		this.displayUnsavedWarning((call) -> {
			if (call) {
				...
				if (this.parent.oriscale != -1) {
					Minecraft.getMinecraft().gameSettings.guiScale = this.parent.oriscale;
	...
	public static class LayoutPropertiesContextMenu extends FMContextMenu {
		...
		public void openMenuAt(int x, int y, int screenWidth, int screenHeight) {
			...
			AdvancedButton randomModeButton = new AdvancedButton(0, 0, 0, 16, randomModeString, true, (press) -> {
				if (this.parent.randomMode) {
					((AdvancedButton)press).displayString = Locals.localize("fancymenu.helper.creator.layoutoptions.randommode.off");
					this.parent.randomMode = false;
				} else {
					((AdvancedButton)press).displayString = Locals.localize("fancymenu.helper.creator.layoutoptions.randommode.on");
			...
			AdvancedButton randomModeFirstTimeButton = new AdvancedButton(0, 0, 0, 16, randomModeFirstTimeString, true, (press) -> {
				if (this.parent.randomOnlyFirstTime) {
					((AdvancedButton)press).displayString = Locals.localize("fancymenu.helper.creator.layoutoptions.randommode.onlyfirsttime.off");
					this.parent.randomOnlyFirstTime = false;
				} else {
					((AdvancedButton)press).displayString = Locals.localize("fancymenu.helper.creator.layoutoptions.randommode.onlyfirsttime.on");
			...
			this.renderingOrderBackgroundButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("helper.creator.layoutoptions.renderorder.background"), true, (press) -> {
				...
				this.renderingOrderForegroundButton.displayString = Locals.localize("helper.creator.layoutoptions.renderorder.foreground");
			...
			this.renderingOrderForegroundButton = new AdvancedButton(0, 0, 0, 16, "§a" + Locals.localize("helper.creator.layoutoptions.renderorder.foreground"), true, (press) -> {
				...
				this.renderingOrderBackgroundButton.displayString = Locals.localize("helper.creator.layoutoptions.renderorder.background");
			...
			if (this.parent.renderorder.equals("background")) {
				renderingOrderForegroundButton.displayString = Locals.localize("helper.creator.layoutoptions.renderorder.foreground");
				renderingOrderBackgroundButton.displayString = "§a" + Locals.localize("helper.creator.layoutoptions.renderorder.background");
			}
					...
					if (call != null) {
						if (call.length() < 3) {
							...
							if (!f.exists() || !f.getAbsolutePath().replace("\\", "/").startsWith(Minecraft.getMinecraft().mcDataDir.getAbsolutePath().replace("\\", "/"))) {
								f = new File(Minecraft.getMinecraft().mcDataDir.getAbsolutePath().replace("\\", "/") + "/" + call);
							}
		...
		public void openMenuAt(int x, int y, int screenWidth, int screenHeight) {
			...
			if (this.parent.audio.isEmpty()) {
				...
				for (Map.Entry<String, Boolean> m : this.parent.audio.entrySet()) {
					...
					if (Minecraft.getMinecraft().fontRenderer.getStringWidth(label) > 200) {
						label = Minecraft.getMinecraft().fontRenderer.trimStringToWidth(label, 200) + "..";
					}
					...
					AdvancedButton toggleLoopButton = new AdvancedButton(0, 0, 0, 16, lab, true, (press2) -> {
						if (((AdvancedButton)press2).displayString.equals(Locals.localize("helper.editor.ui.element.manageaudio.loop.off"))) {
							SoundHandler.setLooped(m.getKey(), true);
							this.parent.audio.put(m.getKey(), true);
							((AdvancedButton)press2).displayString = Locals.localize("helper.editor.ui.element.manageaudio.loop.on");
						} else {
							SoundHandler.setLooped(m.getKey(), false);
							this.parent.audio.put(m.getKey(), false);
							((AdvancedButton)press2).displayString = Locals.localize("helper.editor.ui.element.manageaudio.loop.off");
						}
					});
