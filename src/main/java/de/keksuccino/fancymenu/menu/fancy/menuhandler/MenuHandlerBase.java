...
public class MenuHandlerBase {
	...
	protected Map<Widget, LoadingRequirementContainer> vanillaButtonLoadingRequirementContainers = new HashMap<>();
		...
		for (PropertiesSet s : rawLayouts) {
			...
			if (!this.forceDisableCustomMenuTitle) {
				...
				if (cusMenuTitle != null) {
					...
					ScreenTitleHandler.setScreenTitle(e.getGui(), new StringTextComponent(PlaceholderParser.replacePlaceholders(cusMenuTitle)));
				}
			}
	...
	public void onButtonsCached(ButtonCachedEvent e) {
		...
		for (Map.Entry<Widget, LoadingRequirementContainer> m : this.vanillaButtonLoadingRequirementContainers.entrySet()) {
	...
	public void onRenderPost(GuiScreenEvent.DrawScreenEvent.Post e) {
		...
		if ((this.customMenuTitle != null) && !this.forceDisableCustomMenuTitle) {
			ScreenTitleHandler.setScreenTitle(e.getGui(), new StringTextComponent(PlaceholderParser.replacePlaceholders(this.customMenuTitle)));
		}
		...
		if (this.shouldCustomize(s)) {
			...
			if (this.canRenderBackground()) {
				if ((this.backgroundAnimation != null) && this.backgroundAnimation.isReady()) {
					...
					Minecraft.getInstance().textureManager.bind(this.backgroundTexture.getResourceLocation());
					//TODO übernehmen
					RenderSystem.color4f(1.0F, 1.0F, 1.0F, this.backgroundOpacity);
					if (!this.panoramaback) {
						if (!this.keepBackgroundAspectRatio) {
							AbstractGui.blit(matrix, 0, 0, 1.0F, 1.0F, s.width + 1, s.height + 1, s.width + 1, s.height + 1);
						} else {
							...
							if (wfinal < s.width) {
								...
								AbstractGui.blit(matrix, screenCenterX - (wfinal / 2), 0, 1.0F, 1.0F, wfinal + 1, s.height + 1, wfinal + 1, s.height + 1);
							}
						}
					} else {
						int w = this.backgroundTexture.getWidth();
			...
			if (MenuCustomization.isMenuCustomizable(Minecraft.getInstance().screen)) {
				...
				if (c != null) {
					...
					if (c.lastHoverState != w.isHovered()) {
						if (w.isHovered() && c.restartAnimationOnHover) {
							restart = true;
						}
					}
					...
					if (hasCustomBackground) {
						if (w instanceof ImageButton) {
							...
							if (msg != null) {
								AbstractGui.drawCenteredString(e.getMatrixStack(), Minecraft.getInstance().font, msg, w.x + w.getWidth() / 2, w.y + (w.getHeight() - 8) / 2, w.getFGColor() | MathHelper.ceil(e.getAlpha() * 255.0F) << 24);
							}
						}
					...
					if (background.startsWith("animation:")) {
						...
						if (f.isFile()) {
							if (f.getPath().toLowerCase().endsWith(".gif")) {
								...
								if (back != null) {
									Minecraft.getInstance().textureManager.bind(back.getResourceLocation());
									RenderSystem.enableBlend();
	...
	protected boolean vanillaButtonLoadingRequirementsMet(Widget b) {
	...
	public boolean isVanillaButtonHidden(Widget w) {
		for (ButtonData d : this.hidden) {
			if (d.getButton() == w) {
				return true;
			}
