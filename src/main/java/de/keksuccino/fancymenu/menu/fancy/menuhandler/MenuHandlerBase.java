...
public class MenuHandlerBase {
	...
	protected void renderBackground(Screen s) {
					...
					if (w == e.getWidget()) {
						...
						if ((hover != null) && (idle != null)) {
							...
							if ((idleR != null) && (hoverR != null)) {
								...
								AbstractGui.blit(w.x, w.y, 0.0F, 0.0F, w.getWidth(), w.getHeight(), w.getWidth(), w.getHeight());

								if (w instanceof ImageButton) {
									String msg = w.getMessage();
									if (msg != null) {
										w.drawCenteredString(Minecraft.getInstance().fontRenderer, msg, w.x + w.getWidth() / 2, w.y + (w.getHeight() - 8) / 2, w.getFGColor() | MathHelper.ceil(e.getAlpha() * 255.0F) << 24);
									}
								}
