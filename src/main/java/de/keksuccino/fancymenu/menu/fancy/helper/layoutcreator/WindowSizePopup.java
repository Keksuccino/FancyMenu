...
public class WindowSizePopup extends FMPopup {
	...
	public void render(int mouseX, int mouseY, Screen renderIn) {
		...
		renderIn.drawCenteredString(font, Locals.localize("helper.creator.windowsize.currentwidth") + ": " + Minecraft.getInstance().getMainWindow().getWidth(), renderIn.width / 2, (renderIn.height / 2) + 15, Color.WHITE.getRGB());
		renderIn.drawCenteredString(font, Locals.localize("helper.creator.windowsize.currentheight") + ": " + Minecraft.getInstance().getMainWindow().getHeight(), renderIn.width / 2, (renderIn.height / 2) + 30, Color.WHITE.getRGB());
