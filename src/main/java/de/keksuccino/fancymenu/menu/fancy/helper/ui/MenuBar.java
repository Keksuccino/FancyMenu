		...
		if (AnimationHandler.isReady()) {
			if (screen != null) {
				...
				if (this.visible) {
					...
					RenderSystem.scalef(this.getScale(), this.getScale(), this.getScale());
					...
					if (this.expanded) {
						...
						for (AdvancedButton b : this.leftElements.values()) {
							if (b.visible) {
								...
								if (!(b instanceof AdvancedImageButton)) {
									int i = Minecraft.getInstance().fontRenderer.getStringWidth(b.getMessage());
									b.setWidth(i + 12);
								}
								b.x = xl;
								b.y = 0;
						...
						if (right != null) {
							...
							right.render(mouseX, mouseY, partialTicks);
