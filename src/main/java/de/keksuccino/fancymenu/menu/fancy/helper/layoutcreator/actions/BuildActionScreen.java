...
public class BuildActionScreen extends Screen {
    ...
    public BuildActionScreen( Screen parentScreen,  ManageActionsScreen.ActionInstance instanceToEdit,  Consumer<ManageActionsScreen.ActionInstance> callback) {

        super((instanceToEdit != null) ? new StringTextComponent(Locals.localize("fancymenu.editor.action.screens.edit_action")) : new StringTextComponent(Locals.localize("fancymenu.editor.action.screens.add_action")));
        ...
        this.editValueButton = new AdvancedButton(0, 0, 150, 20, Locals.localize("fancymenu.editor.action.screens.build_screen.edit_value"), true, (button) -> {
            TextEditorScreen s = new TextEditorScreen(new StringTextComponent(this.instance.action.getValueDescription()), this, null, (call) -> {
                if (call != null) {
    ...
    protected void setDescription( ButtonActionContainer action) {
        ...
        if ((action != null) && (action.getActionDescription() != null)) {
            for (String s : StringUtils.splitLines(action.getActionDescription(), "%n%")) {
                TextScrollAreaEntry e = new TextScrollAreaEntry(this.actionDescriptionScrollArea, new StringTextComponent(s), (entry) -> {});
    ...
    public static class ActionScrollEntry extends TextListScrollAreaEntry {
        ...
        public ActionScrollEntry(ScrollArea parent,  ButtonActionContainer action,  Consumer<TextListScrollAreaEntry> onClick) {
            super(parent, new StringTextComponent(action.getAction()).setStyle(Style.EMPTY.withColor(Color.fromRgb(TEXT_COLOR_GRAY_1.getRGB()))), LISTING_DOT_BLUE, onClick);
            this.action = action;
        }
