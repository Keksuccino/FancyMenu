...
public class FMContextMenu extends ContextMenu {
	...
	public void openMenuAt(int x, int y, int screenWidth, int screenHeight) {
		...
		int i = 20;
		for (AdvancedButton b : this.content) {
			int sw = Minecraft.getInstance().fontRenderer.getStringWidth(b.getMessage()) + 12;
			if (b.getWidth() > sw) {
				sw = b.getWidth();
			}
			if (sw > i) {
		...
		if (!this.content.isEmpty()) {
			for (Integer i : this.separators) {
				if (this.content.size() >= i+1) {
					...
					UIBase.fill(b.x, b.y, b.x + this.width, b.y + 1, c.getRGB());
				}
			}
		}
	}
	...
	protected void renderBorder() {
		if (!this.content.isEmpty()) {
			...
			//TOP
			UIBase.fill(b.x, b.y, b.x + this.width, b.y + 1, UIBase.getButtonBorderIdleColor().getRGB());
			//LEFT
			UIBase.fill(b.x, b.y + 1, b.x + 1, b.y + this.lastHeight, UIBase.getButtonBorderIdleColor().getRGB());
			//BOTTOM
			UIBase.fill(b.x + 1, b.y + this.lastHeight - 1, b.x + this.width, b.y + this.lastHeight, UIBase.getButtonBorderIdleColor().getRGB());
			//RIGHT
			UIBase.fill(b.x + this.width - 1, b.y + 1, b.x + this.width, b.y + this.lastHeight - 1, UIBase.getButtonBorderIdleColor().getRGB());
		}
	}
