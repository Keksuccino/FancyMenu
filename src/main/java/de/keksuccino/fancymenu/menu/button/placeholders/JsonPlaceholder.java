...
public class JsonPlaceholder extends PlaceholderTextContainer {
    ...
    public JsonPlaceholder() {
        ...
        if (!eventsRegistered) {
            Konkrete.getEventHandler().registerEventsFrom(JsonPlaceholder.class);
                ...
                new Thread(() -> {
                    try {
                        if (WebUtils.isValidUrl(link)) {
                            cachedPlaceholders.put(placeholder, JsonUtils.getJsonValueByPath(new URL(link), jsonPath));
                        } else {
