        ...
        Minecraft.getInstance().execute(() -> {
            try {
                if (getOrSet.equalsIgnoreCase("get")) {
                    ...
                    if (s != null) {
                        stack.sendFeedback(new StringTextComponent(Locals.localize("fancymenu.commands.variable.get.success", s)), false);
                    } else {
                        stack.sendErrorMessage(new StringTextComponent(Locals.localize("fancymenu.commands.variable.not_found")));
                    }
                }
            } catch (Exception e) {
        ...
        Minecraft.getInstance().execute(() -> {
            try {
                if (getOrSet.equalsIgnoreCase("set")) {
                    ...
                    if (sendFeedback) {
                        stack.sendFeedback(new StringTextComponent(Locals.localize("fancymenu.commands.variable.set.success", setToValue)), false);
                    }
                }
            } catch (Exception e) {
                stack.sendErrorMessage(new StringTextComponent("Error while executing command!"));
