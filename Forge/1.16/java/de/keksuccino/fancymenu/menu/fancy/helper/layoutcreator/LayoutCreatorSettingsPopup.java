...
public class LayoutCreatorSettingsPopup extends Popup {
	...
	public LayoutCreatorSettingsPopup(LayoutCreatorScreen parent) {
		...
		this.vanillaNotificationButton = new AdvancedButton(0, 0, 100, 20, vanillaNotifyLabel, true, (press) -> {
			try {
				if (FancyMenu.config.getOrDefault("showvanillamovewarning", true)) {
					...
					press.setMessage(new StringTextComponent(Locals.localize("helper.creator.sidemenu.settings.off")));
				} else {
					FancyMenu.config.setValue("showvanillamovewarning", true);
					press.setMessage(new StringTextComponent(Locals.localize("helper.creator.sidemenu.settings.on")));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		this.popupMenuScaleTextfield = new AdvancedTextField(Minecraft.getInstance().fontRenderer, 0, 0, 98, 20, true, CharacterFilter.getDoubleCharacterFiler());
	...
	public void render(MatrixStack matrix, int mouseX, int mouseY, Screen renderIn) {
		...
		if (this.isDisplayed()) {
			...
			drawString(matrix, font, vanillaNotify, midX - vanillaNotifyWidth - 20, midY - 75, Color.WHITE.getRGB());
			...
			drawString(matrix, font, popScale, midX - popScaleWidth - 20, midY - 50, Color.WHITE.getRGB());
			
			this.popupMenuScaleTextfield.x = midX + 20 + 1;
			this.popupMenuScaleTextfield.y = midY - 55;
			this.popupMenuScaleTextfield.render(matrix, mouseX, mouseY, Minecraft.getInstance().getRenderPartialTicks());
