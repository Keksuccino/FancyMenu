...
public class ButtonCache {
	...
	private static List<ButtonData> cacheButtons(GuiScreen s, int screenWidth, int screenHeight) {
		...
		try {
			...
			//fontRenderer field
			Field f2 = net.minecraftforge.fml.relauncher.ReflectionHelper.findField(GuiScreen.class, "fontRendererObj", "field_146289_q");
			f2.set(s, Minecraft.getMinecraft().fontRendererObj);
			...
			for (GuiButton w : (List<GuiButton>) f.get(s)) {
				String idRaw = w.xPosition + "" + w.yPosition;
				long id = 0;
	...
	public static AdvancedButton convertToAdvancedButton(long id, boolean handleClick) {
		...
		if ((d != null) && !(d.getButton() instanceof AdvancedButton)) {
			AdvancedButton b = new AdvancedButton(d.getButton().xPosition, d.getButton().yPosition, d.getButton().width, d.getButton().height, d.getButton().displayString, handleClick, (press) -> {
...
//			d.getButton().visible = false;
	...
	private static List<GuiButton> sortButtons(List<GuiButton> widgets) {
		...
		for (GuiButton w : widgets) {
			...
			if (m.containsKey(w.yPosition)) {
				m.get(w.yPosition).add(w);
			} else {
				m.put(w.yPosition, new ArrayList<GuiButton>());
				m.get(w.yPosition).add(w);
			}
		}
