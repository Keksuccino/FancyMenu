...
public class LanguageMenuHandler extends MenuHandlerBase {
	...
	public void drawToBackground(BackgroundDrawnEvent e) {
		...
		if (this.shouldCustomize(e.gui) && MenuCustomization.isMenuCustomizable(e.gui)) {
			if (this.list != null) {
				this.list.drawScreen(MouseInput.getMouseX(), MouseInput.getMouseY(), RenderUtils.getRenderPartialTicks());
			}
			e.gui.drawCenteredString(Minecraft.getMinecraft().fontRendererObj, I18n.format("options.language"), e.gui.width / 2, 16, 16777215);
	        e.gui.drawCenteredString(Minecraft.getMinecraft().fontRendererObj, "(" + I18n.format("options.languageWarning") + ")", e.gui.width / 2, e.gui.height - 56, 8421504);

			if (this.currentButtons != null) {
				for(int i = 0; i < this.currentButtons.size(); ++i) {
					this.currentButtons.get(i).drawButton(Minecraft.getMinecraft(), MouseInput.getMouseX(), MouseInput.getMouseY());
				}
			}
		}
	}
    ...
    class List extends GuiSlot {
        ...
        protected void elementClicked(int slotIndex, boolean isDoubleClick, int mouseX, int mouseY) {
            ...
            Minecraft.getMinecraft().fontRendererObj.setUnicodeFlag(Minecraft.getMinecraft().getLanguageManager().isCurrentLocaleUnicode() || Minecraft.getMinecraft().gameSettings.forceUnicodeFont);
            Minecraft.getMinecraft().fontRendererObj.setBidiFlag(Minecraft.getMinecraft().getLanguageManager().isCurrentLanguageBidirectional());
        ...
        protected void drawSlot(int slotIndex, int xPos, int yPos, int heightIn, int mouseXIn, int mouseYIn) {
            ...
            LanguageMenuHandler.this.current.drawCenteredString(Minecraft.getMinecraft().fontRendererObj, ((Language)this.languageMap.get(this.langCodeList.get(slotIndex))).toString(), this.width / 2, yPos + 1, 16777215);
            Minecraft.getMinecraft().fontRendererObj.setBidiFlag(Minecraft.getMinecraft().getLanguageManager().getCurrentLanguage().isBidirectional());
        }
