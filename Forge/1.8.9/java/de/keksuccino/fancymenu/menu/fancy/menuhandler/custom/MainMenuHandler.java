	...
	public void drawToBackground(BackgroundDrawnEvent e) {
		if (this.shouldCustomize(e.gui)) {
			...
			if (!this.canRenderBackground()) {
				this.panoramaTimer += RenderUtils.getRenderPartialTicks();
				this.renderSkybox(mouseX, mouseY, RenderUtils.getRenderPartialTicks(), e.gui);
			}
			...
			if (!FancyMenu.config.getOrDefault("hidelogo", true)) {
				...
				if ((double) minecraftLogoSpelling < 1.0E-4D) {
					e.gui.drawTexturedModalRect(j + 0, 30, 0, 0, 99, 44);
					e.gui.drawTexturedModalRect(j + 99, 30, 129, 0, 27, 44);
					e.gui.drawTexturedModalRect(j + 99 + 26, 30, 126, 0, 3, 44);
					e.gui.drawTexturedModalRect(j + 99 + 26 + 3, 30, 99, 0, 26, 44);
					e.gui.drawTexturedModalRect(j + 155, 30, 0, 45, 155, 44);
				} else {
					e.gui.drawTexturedModalRect(j + 0, 30, 0, 0, 155, 44);
					e.gui.drawTexturedModalRect(j + 155, 30, 0, 45, 155, 44);
				}
			...
			if (!FancyMenu.config.getOrDefault("hidebranding", true)) {
		        ...
		        for (int brdline = 0; brdline < brandings.size(); brdline++) {
		            ...
		            if (!Strings.isNullOrEmpty(brd)) {
		                e.gui.drawString(Minecraft.getMinecraft().fontRendererObj, brd, 2, e.gui.height - ( 10 + brdline * (Minecraft.getMinecraft().fontRendererObj.FONT_HEIGHT + 1)), 16777215);
		            }
		        }
			}
			
			if (!FancyMenu.config.getOrDefault("hideforgenotifications", false)) {
				ForgeHooksClient.renderMainMenu((GuiMainMenu) e.gui, font, e.gui.width, e.gui.height);
			}
			...
			int cX = 0;
			int cY = 0;
			int cW = Minecraft.getMinecraft().fontRendererObj.getStringWidth(c);
			int cH = 10;
			...
			e.gui.drawString(font, c, cX, cY, -1);
			
			//TODO (wird in 1.8.9 eigentlich überhaupt nicht gemacht, also eventuell einfach rausnehmen)
//			if ((mouseX >= cX) && (mouseX <= cX + cW) && (mouseY >= cY) && (mouseY <= cY + cH)) {
//				Gui.drawRect(cX, cY + cH - 1, cX + cW, cY + cH, -1);
//				
//				if (MouseInput.isLeftMouseDown()) {
//					Minecraft.getMinecraft().displayGuiScreen(new GuiWinGame());
//				}
//			}
			...
			if (!FancyMenu.config.getOrDefault("hiderealmsnotifications", false)) {
				this.drawRealmsNotification(e.gui);
			}
			...
			if (!FancyMenu.config.getOrDefault("hidesplashtext", true)) {
				...
				e.gui.drawCenteredString(font, this.splash, 0, -8, -256);
	...
	private void renderButtonsAndLabels(GuiScreenEvent.BackgroundDrawnEvent e, int mouseX, int mouseY) {
		List<GuiButton> buttons = this.getButtonList(e.gui);
		List<GuiLabel> labels = this.getLabelList(e.gui);
		
		if (buttons != null) {
			for(int i = 0; i < buttons.size(); ++i) {
				buttons.get(i).drawButton(Minecraft.getMinecraft(), mouseX, mouseY);
			}
		}
	...
	private void drawRealmsNotification(GuiScreen gui) {
		if (Minecraft.getMinecraft().gameSettings.getOptionOrdinalValue(GameSettings.Options.REALMS_NOTIFICATIONS)) {
			...
			if (realms != null) {
				realms.drawScreen((int)MouseInput.getMouseX(), (int)MouseInput.getMouseY(), RenderUtils.getRenderPartialTicks());
			}
		}
	}
...
//	}
	...
	private void renderFooter(GuiScreenEvent.DrawScreenEvent e) {
		...
		if (tickFooter < 30) {
			tickFooter += 1;
		} else if (e.gui.height >= 280) {
			int i = MathHelper.ceiling_double_int(fadeFooter * 255.0F) << 24;
			
			RenderUtils.setScale(1.1F);
			e.gui.drawCenteredString(Minecraft.getMinecraft().fontRendererObj, "§fDISCOVER MORE AT MINECRAFT.NET", (int) (e.gui.width / 2 / 1.1D), (int) ((e.gui.height - 50) / 1.1D), i);
			RenderUtils.postScale();
			
			RenderUtils.setScale(0.7F);
			e.gui.drawString(Minecraft.getMinecraft().fontRendererObj, "§f@MINECRAFT", (int) ((e.gui.width / 2 - 10) / 0.7D), (int) ((e.gui.height - 30) / 0.7D), i);
			
			e.gui.drawString(Minecraft.getMinecraft().fontRendererObj, "§fMINECRAFT", (int) ((e.gui.width / 2 + 60) / 0.7D), (int) ((e.gui.height - 30) / 0.7D), i);

			e.gui.drawString(Minecraft.getMinecraft().fontRendererObj, "§f/MINECRAFT", (int) ((e.gui.width / 2 - 80) / 0.7D), (int) ((e.gui.height - 30) / 0.7D), i);
			RenderUtils.postScale();
			...
			GlStateManager.color(1.0F, 1.0F, 1.0F, MathHelper.clamp_float(fadeFooter, 0.1F, 1.0F));
			GuiIngame.drawModalRectWithCustomSizedTexture(e.gui.width / 2 - 100, e.gui.height - 35, 0.0F, 0.0F, 15, 15, 15, 15);

			Minecraft.getMinecraft().getTextureManager().bindTexture(TWITTER);
			GlStateManager.blendFunc(770, 771);
			GlStateManager.color(1.0F, 1.0F, 1.0F, MathHelper.clamp_float(fadeFooter, 0.1F, 1.0F));
			GuiIngame.drawModalRectWithCustomSizedTexture(e.gui.width / 2 - 30, e.gui.height - 35, 0.0F, 0.0F, 15, 15, 15, 15);

			Minecraft.getMinecraft().getTextureManager().bindTexture(INSTAGRAM);
			GlStateManager.blendFunc(770, 771);
			GlStateManager.color(1.0F, 1.0F, 1.0F, MathHelper.clamp_float(fadeFooter, 0.1F, 1.0F));
			GuiIngame.drawModalRectWithCustomSizedTexture(e.gui.width / 2 + 40, e.gui.height - 35, 0.0F, 0.0F, 15, 15, 15, 15);
