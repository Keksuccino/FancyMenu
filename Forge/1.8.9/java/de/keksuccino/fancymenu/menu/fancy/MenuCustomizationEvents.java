...
public class MenuCustomizationEvents {
	...
	public void onInitPre(GuiScreenEvent.InitGuiEvent.Pre e) {
		...
		if (MenuCustomization.isValidScreen(e.gui) && !LayoutCreatorScreen.isActive) {
			this.idle = false;
		}
		
		if (MenuCustomization.isValidScreen(e.gui) && !MenuCustomization.isMenuCustomizable(e.gui) && !(e.gui instanceof LayoutCreatorScreen)) {
		...
		if (!FancyMenu.config.getOrDefault("playmenumusic", true)) {
			//getMusicTicker()
			MusicTicker m = Minecraft.getMinecraft().func_181535_r();
