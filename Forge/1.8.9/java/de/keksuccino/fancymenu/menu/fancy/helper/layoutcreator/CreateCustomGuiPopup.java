...
public class CreateCustomGuiPopup extends Popup {
	...
	public CreateCustomGuiPopup() {
		...
		this.identifierText = new AdvancedTextField(Minecraft.getMinecraft().fontRendererObj, 0, 0, 200, 20, true, null);
		
		this.titleText = new AdvancedTextField(Minecraft.getMinecraft().fontRendererObj, 0, 0, 200, 20, true, null);
	...
	public void render(int mouseX, int mouseY, GuiScreen renderIn) {
		...
		this.identifierText.xPosition = (renderIn.width / 2) - (this.identifierText.getWidth() / 2);
		this.identifierText.yPosition = (renderIn.height / 2) - 65;
		...
		this.titleText.xPosition = (renderIn.width / 2) - (this.titleText.getWidth() / 2);
		this.titleText.yPosition = (renderIn.height / 2) - 22;
		...
		this.allowEscButton.xPosition = (renderIn.width / 2) - this.allowEscButton.width - 5;
		this.allowEscButton.yPosition = (renderIn.height / 2) + 20;
		
		this.doNotAllowEscButton.xPosition = (renderIn.width / 2) + 5;
		this.doNotAllowEscButton.yPosition = (renderIn.height / 2) + 20;
		//----------
		
		this.cancelButton.xPosition = (renderIn.width / 2) - (this.cancelButton.width / 2);
		this.cancelButton.yPosition = (renderIn.height / 2) + 55;
		...
		if ((id != null) && (id.length() > 0) && !id.contains(" ")) {
			...
			if (!l.contains(id)) {
				...
				this.doneButton.yPosition = (renderIn.height / 2) + 80;
		...
		try {
			...
			if (this.identifierText.getText() != null) {
				...
				if (gui != null) {
					...
					Minecraft.getMinecraft().displayGuiScreen(new LayoutCreatorScreen(gui));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
