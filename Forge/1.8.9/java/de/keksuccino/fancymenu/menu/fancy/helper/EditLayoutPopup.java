...
public class EditLayoutPopup extends Popup {
	...
	public void render(int mouseX, int mouseY, GuiScreen renderIn) {
		...
		if (this.isDisplayed()) {
			...
			renderIn.drawCenteredString(Minecraft.getMinecraft().fontRendererObj, "§l" + Locals.localize("helper.creator.editlayout"),
					renderIn.width / 2, (renderIn.height / 2) - 110, Color.WHITE.getRGB());
			
			int i = 0;
			for (String s : this.msg) {
				renderIn.drawCenteredString(Minecraft.getMinecraft().fontRendererObj, s, renderIn.width / 2, (renderIn.height / 2) - 90 + i, Color.WHITE.getRGB());
				i += 10;
			}
			
			int startY = (renderIn.height / 2) - 70 + i;
			
			this.loadAllButton.xPosition = (renderIn.width / 2) - (this.loadAllButton.width / 2);
			this.loadAllButton.yPosition = startY;
			
			
			renderIn.drawCenteredString(Minecraft.getMinecraft().fontRendererObj, Locals.localize("helper.creator.editlayout.popup.layouts"),
			...
			renderIn.drawCenteredString(Minecraft.getMinecraft().fontRendererObj,
					"(" + Locals.localize("helper.creator.editlayout.popup.actionscount", "" + count) + ")",
					renderIn.width / 2, startY + 75, Color.WHITE.getRGB());

			
			this.loadLayoutButton.xPosition = (renderIn.width / 2) - this.loadLayoutButton.width - 5;
			this.loadLayoutButton.yPosition = startY + 95;
			
			this.cancelButton.xPosition = (renderIn.width / 2) + 5;
			this.cancelButton.yPosition = startY + 95;
