...
public abstract class LayoutObject extends Gui {
	...
	protected void init() {
		...
		this.orientationButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("helper.creator.items.setorientation"), true, (press) -> {
			this.orientationMenu.openMenuAt(press.xPosition + press.width, press.yPosition);
		});
		...
		this.layersButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("helper.creator.items.chooselayer"), true, (press) -> {
			...
			for (LayoutObject o : this.hoveredLayers) {
				...
				if (label == null) {
					...
					if (Minecraft.getMinecraft().fontRendererObj.getStringWidth(label) > 90) {
						label = Minecraft.getMinecraft().fontRendererObj.trimStringToWidth(label, 85) + "..";
					}
				}
			...
			this.layersPopup.openMenuAt(press.xPosition + press.width, press.yPosition);
		});
		...
		stretchBtn = new AdvancedButton(0, 0, 0, 0, Locals.localize("helper.creator.object.stretch"), true, (press) -> {
			this.stretchPopup.openMenuAt(0, press.yPosition);
		});
	...
	protected void renderBorder(int mouseX, int mouseY) {
		...
		this.drawString(Minecraft.getMinecraft().fontRendererObj, Locals.localize("helper.creator.items.border.orientation") + ": " + this.object.orientation, this.object.getPosX(handler)*2, (this.object.getPosY(handler)*2) - 26, Color.WHITE.getRGB());
		this.drawString(Minecraft.getMinecraft().fontRendererObj, Locals.localize("helper.creator.items.border.posx") + ": " + this.object.getPosX(handler), this.object.getPosX(handler)*2, (this.object.getPosY(handler)*2) - 17, Color.WHITE.getRGB());
		this.drawString(Minecraft.getMinecraft().fontRendererObj, Locals.localize("helper.creator.items.border.width") + ": " + this.object.width, this.object.getPosX(handler)*2, (this.object.getPosY(handler)*2) - 8, Color.WHITE.getRGB());
		
		this.drawString(Minecraft.getMinecraft().fontRendererObj, Locals.localize("helper.creator.items.border.posy") + ": " + this.object.getPosY(handler), ((this.object.getPosX(handler) + this.object.width)*2)+3, ((this.object.getPosY(handler) + this.object.height)*2) - 14, Color.WHITE.getRGB());
		this.drawString(Minecraft.getMinecraft().fontRendererObj, Locals.localize("helper.creator.items.border.height") + ": " + this.object.height, ((this.object.getPosX(handler) + this.object.width)*2)+3, ((this.object.getPosY(handler) + this.object.height)*2) - 5, Color.WHITE.getRGB());
