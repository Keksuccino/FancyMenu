...
import de.keksuccino.fancymenu.menu.entity.PlayerEntityCache;
...
public class PlayerEntityCustomizationItem extends CustomizationItemBase {
	...
	private static final World DUMMY_WORLD = getDummyWorld();
//	private static final BlockPos BLOCK_POS = new BlockPos(0, 0, 0);
	...
	public PlayerEntityCustomizationItem(PropertiesSection item) {
		...
		if (this.playerName != null) {
			this.playerName = MenuCustomization.convertString(this.playerName);
		}
		...
		if (skinUrl != null) {
			skinUrl = MenuCustomization.convertString(skinUrl);
		...
		if ((skin != null) && (this.entity.skinLocation == null)) {
			skin = MenuCustomization.convertString(skin);
		...
		if (capeUrl != null) {
			capeUrl = MenuCustomization.convertString(capeUrl);
		...
		if ((cape != null) && (this.entity.capeLocation == null)) {
			cape = MenuCustomization.convertString(cape);
		...
		if (rotX != null) {
			rotX = MenuCustomization.convertString(rotX);
			rotX = rotX.replace(" ", "");
		...
		if (rotY != null) {
			rotY = MenuCustomization.convertString(rotY);
			rotY = rotY.replace(" ", "");
		...
		if (bodyrotX != null) {
			bodyrotX = MenuCustomization.convertString(bodyrotX);
		...
		if (bodyrotY != null) {
			bodyrotY = MenuCustomization.convertString(bodyrotY);
	...
	private static void renderAsFancy(Runnable renderFancy) {
		...
		if (b) {
			...
			GameSettings g = Minecraft.getInstance().gameSettings;
	...
	private static World getDummyWorld() {
		DimensionType t = DimensionType.OVERWORLD;
		World w = new World(new WorldInfo(new WorldSettings(1, GameType.SURVIVAL, false, false, WorldType.DEFAULT), ""), t, PlayerEntityCustomizationItem::getDummyChunkProvider, null, false) {
			@Override
	...
	public static class MenuPlayerRenderer extends LivingRenderer<MenuPlayerEntity, PlayerModel<MenuPlayerEntity>> {
		...
		@Override
		protected void renderName(MenuPlayerEntity playerEntity, String displayNameIn, MatrixStack matrix, IRenderTypeBuffer bufferIn, int packedLightIn) {
			if (playerEntity.showName) {
				...
				fontrenderer.renderString(displayNameIn, f2, 0, 553648127, false, matrix4f, bufferIn, flag, j, packedLightIn);
				if (flag) {
		...
		protected void applyRotations(MenuPlayerEntity entityLiving, MatrixStack matrixStackIn, float ageInTicks, float rotationYaw, float partialTicks) {
			...
			if (entityLiving.isElytraFlying()) {
				...
				Vec3d vector3d = entityLiving.getLook(partialTicks);
				Vec3d vector3d1 = entityLiving.getMotion();
	...
	public static class MenuPlayerEntity extends PlayerEntity {
		...
		public MenuPlayerEntity(String playerName) {
			super(DUMMY_WORLD, new GameProfile(PlayerEntity.getOfflineUUID(getRawPlayerName(playerName)), getRawPlayerName(playerName)));
			if (playerName != null) {
		...
		public ITextComponent getDisplayName() {
			...
			return new StringTextComponent("steve");
		}
