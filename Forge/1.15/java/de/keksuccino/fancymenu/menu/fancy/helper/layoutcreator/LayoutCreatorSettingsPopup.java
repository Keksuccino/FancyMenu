...
public class LayoutCreatorSettingsPopup extends Popup {
		...
		this.vanillaNotificationButton = new AdvancedButton(0, 0, 100, 20, vanillaNotifyLabel, true, (press) -> {
			try {
				if (FancyMenu.config.getOrDefault("showvanillamovewarning", true)) {
					...
					press.setMessage(Locals.localize("helper.creator.sidemenu.settings.off"));
				} else {
					FancyMenu.config.setValue("showvanillamovewarning", true);
					press.setMessage(Locals.localize("helper.creator.sidemenu.settings.on"));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void render(int mouseX, int mouseY, Screen renderIn) {
		...
		if (this.isDisplayed()) {
			...
			fill(midX - 1, midY - 100, midX + 1, midY + 60, new Color(255, 255, 255, 100).getRGB());
			...
			this.popupMenuScaleTextfield.render(mouseX, mouseY, Minecraft.getInstance().getRenderPartialTicks());
