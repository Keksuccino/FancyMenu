...
public class WindowSizePopup extends Popup {
	...
	public void render(int mouseX, int mouseY, Screen renderIn) {
		...
		if (this.type == ActionType.BIGGERTHAN) {
			drawCenteredString(font, "§l" + Locals.localize("helper.creator.windowsize.biggerthan.desc"), renderIn.width / 2, (renderIn.height / 2) - 110, Color.WHITE.getRGB());
		} else {
			drawCenteredString(font, "§l" + Locals.localize("helper.creator.windowsize.smallerthan.desc"), renderIn.width / 2, (renderIn.height / 2) - 110, Color.WHITE.getRGB());
		}
		
		drawCenteredString(font, Locals.localize("general.width"), renderIn.width / 2, (renderIn.height / 2) - 80, Color.WHITE.getRGB());
		this.widthText.x = (renderIn.width / 2) - (this.widthText.getWidth() / 2);
		this.widthText.y = (renderIn.height / 2) - 65;
		this.widthText.render(mouseX, mouseY, partial);
		
		drawCenteredString(font, Locals.localize("general.height"), renderIn.width / 2, (renderIn.height / 2) - 37, Color.WHITE.getRGB());
		this.heightText.x = (renderIn.width / 2) - (this.heightText.getWidth() / 2);
		this.heightText.y = (renderIn.height / 2) - 22;
		this.heightText.render(mouseX, mouseY, partial);

		drawCenteredString(font, Locals.localize("helper.creator.windowsize.currentwidth") + ": " + Minecraft.getInstance().getMainWindow().getWidth(), renderIn.width / 2, (renderIn.height / 2) + 15, Color.WHITE.getRGB());
		drawCenteredString(font, Locals.localize("helper.creator.windowsize.currentheight") + ": " + Minecraft.getInstance().getMainWindow().getHeight(), renderIn.width / 2, (renderIn.height / 2) + 30, Color.WHITE.getRGB());
