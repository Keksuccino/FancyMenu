...
public class ExternalTextureSlideshowRenderer extends AbstractGui {
	...
	protected void renderPrevious() {
		if ((this.previous != null) && (this.current != this.previous)) {
			...
			Minecraft.getInstance().getTextureManager().bindTexture(this.previous.getResourceLocation());
	...
	protected void renderCurrent() {
		if (this.current != null) {
			...
			RenderSystem.enableBlend();
			RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
			Minecraft.getInstance().getTextureManager().bindTexture(this.current.getResourceLocation());
			blit(this.x, this.y, 0.0F, 0.0F, this.width, this.height, this.width, this.height);
		}
	}
