...
public class ExternalTextureSlideshowRenderer extends Gui {
	...
	protected void renderPrevious() {
		if ((this.previous != null) && (this.current != this.previous)) {
			...
			Minecraft.getMinecraft().getTextureManager().bindTexture(this.previous.getResourceLocation());
	...
	protected void renderCurrent() {
		if (this.current != null) {
			...
			GlStateManager.enableBlend();
			GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
			Minecraft.getMinecraft().getTextureManager().bindTexture(this.current.getResourceLocation());
			drawModalRectWithCustomSizedTexture(this.x, this.y, 0.0F, 0.0F, this.width, this.height, this.width, this.height);
		}
	}
